    ...
    MapCoordDelta max_dist, HitPoints max_damage, long blow_strength, struct ShotConfigStats* shotst)
{
    ...
    if (nowibble_line_of_sight_3d(pos, &tngdst->mappos))
    {
        ...
        if ((tngdst->class_id == TCls_Creature) && (tngdst->owner == owner)) {
            max_dist = max_dist * game.conf.rules[TODO_SO_ATM_0].magic.friendly_fight_area_range_percent / 100;
            max_damage = max_damage * game.conf.rules[TODO_SO_ATM_0].magic.friendly_fight_area_damage_percent / 100;
        }
        ...
        if (distance <= max_dist)
        {
            if (tngdst->class_id == TCls_Creature)
            {
                ...
                if (tngdst->health < 0)
                {
                    ...
                    if ((players_creatures_tolerate_each_other(tngsrc->owner,tngdst->owner) && !flag_is_set(game.conf.rules[TODO_SO_ATM_0].game.classic_bugs_flags,ClscBug_FriendlyFaint)) || (flag_is_set(shot_model_flags,ShMF_NoStun)) )
                    {
