...
void gui_area_event_button(struct GuiButton *gbtn)
{
    if ((gbtn->flags & LbBtnF_Enabled) != 0)
    {
        ...
        else if ((i <= EVENT_BUTTONS_COUNT) && (dungeon->event_button_index[i] == dungeon->visible_event_idx))
        {
            draw_gui_panel_sprite_left(gbtn->scr_pos_x, gbtn->scr_pos_y, ps_units_per_px, gbtn->sprite_idx);
        }
        else
...
void gui_area_progress_bar_short(struct GuiButton *gbtn, int units_per_px, int progress, int total)
{
    ...
    if (total > 0)
    {
        ...
        else if (bar_fill > BAR_FULL_WIDTH)
        {
...
void gui_area_big_room_button(struct GuiButton *gbtn)
{
    ...
    {
        if ((player->work_state == PSt_BuildRoom) && (player->chosen_room_kind == game.chosen_room_kind) && ((game.play_gameturn & 1) == 0))
        {
...
void gui_area_spell_button(struct GuiButton *gbtn)
{
    ...
    if ((dungeon->magic_resrchable[pwkind]) || (dungeon->magic_level[pwkind] > 0))
    {
        ...
        if ((gbtn->flags & LbBtnF_Enabled) != 0)
        {
            ...
            if (((i == PSt_CallToArms) && player_uses_power_call_to_arms(my_player_number)) || ((i == PSt_SightOfEvil) && player_uses_power_sight(my_player_number)) || ((pwkind == PwrK_OBEY) && player_uses_power_obey(my_player_number)))
            {
            ...
            if ((gbtn->gbactn_1 == 0) && (gbtn->gbactn_2 == 0))
            {
                if ((((i != PSt_CallToArms) || !player_uses_power_call_to_arms(my_player_number)) && ((i != PSt_SightOfEvil) || !player_uses_power_sight(my_player_number))) || ((game.play_gameturn & 1) == 0))
                {
                    draw_gui_panel_sprite_left(gbtn->scr_pos_x, gbtn->scr_pos_y, ps_units_per_px, spr_idx);
                    drawn = true;
                }
...
void gui_area_trap_button(struct GuiButton *gbtn)
{
    ...
    {
        if (!is_trap_buildable(my_player_number, manufctr->tngmodel) && !is_trap_placeable(my_player_number, manufctr->tngmodel) && !is_trap_built(my_player_number, manufctr->tngmodel))
        {
            lbDisplay.DrawFlags = flg_mem;
            return;
        }
    ...
    {
        if (!is_door_buildable(my_player_number, manufctr->tngmodel) && !is_door_placeable(my_player_number, manufctr->tngmodel) && !is_door_built(my_player_number, manufctr->tngmodel))
        {
            lbDisplay.DrawFlags = flg_mem;
            return;
        }
...
void gui_area_big_trap_button(struct GuiButton *gbtn)
{
    ...
    else if ((((manufctr->tngclass == TCls_Trap) && (player->chosen_trap_kind == manufctr->tngmodel) && (player->work_state == PSt_PlaceTrap)) || ((manufctr->tngclass == TCls_Door) && (player->chosen_door_kind == manufctr->tngmodel) && (player->work_state == PSt_PlaceDoor))) && ((game.play_gameturn & 1) == 0))
    {
        draw_gui_panel_sprite_rmleft(gbtn->scr_pos_x - 4 * units_per_px / 16, gbtn->scr_pos_y - 32 * units_per_px / 16, ps_units_per_px, gbtn->sprite_idx, 44);
    }
    else
...
void maintain_big_trap(struct GuiButton *gbtn)
{
    ...
    if (((manufctr->tngclass == TCls_Trap) && is_trap_placeable(my_player_number, manufctr->tngmodel)) || ((manufctr->tngclass == TCls_Door) && is_door_placeable(my_player_number, manufctr->tngmodel)))
    {
        gbtn->btype_value &= LbBFeF_IntValueMask;
        gbtn->flags |= LbBtnF_Enabled;
    }
...
void gui_area_experience_button(struct GuiButton *gbtn)
{
    ...
    else if (thing_is_dead_creature(ctrltng))
    {
        draw_gui_panel_sprite_left(gbtn->scr_pos_x, gbtn->scr_pos_y, ps_units_per_px, gbtn->sprite_idx);
...
void maintain_event_button(struct GuiButton *gbtn)
{
    ...
    if ((dungeon->visible_event_idx != 0) && (evidx == dungeon->visible_event_idx))
    {
        ...
        if (is_game_key_pressed(Gkey_ToggleMessage, &keycode, false) && ((get_player(my_player_number)->allocflags & PlaF_NewMPMessage) == 0))
        {
            gui_kill_event(gbtn);
    ...
    if (((event->kind == EvKind_FriendlyFight) || (event->kind == EvKind_EnemyFight)) && ((event->mappos_x != 0) || (event->mappos_y != 0)) && ((game.play_gameturn & 0x01) != 0))
    {
    ...
    else if (((event->kind == EvKind_Information) || (event->kind == EvKind_QuickInformation)) && (event->target < 0) && ((game.play_gameturn & 0x01) != 0))
    {
...
void gui_area_player_creature_info(struct GuiButton *gbtn)
{
    ...
    {
        if (((dungeon->num_active_creatrs < dungeon->max_creatures_attracted) && (!game.pool.is_empty)) || ((game.play_gameturn & 1) != 0))
        {
            draw_gui_panel_sprite_left(gbtn->scr_pos_x, gbtn->scr_pos_y, ps_units_per_px, gbtn->sprite_idx);
        }
        else
