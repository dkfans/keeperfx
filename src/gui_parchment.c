...
long get_parchment_background_area_rect(struct TbRect *bkgnd_area)
{
    ...
    if (bkgnd_area->bottom > rect_h)
    {
        bkgnd_area->bottom = rect_h;
    }
    return units_per_px;
}
...
TbBool point_to_overhead_map(const struct Camera *camera, const long screen_x, const long screen_y, long *map_x, long *map_y)
{
    ...
    *map_x = 0;
    *map_y = 0;
    if ((screen_x >= map_area.left) && (screen_x < map_area.right) && (screen_y >= map_area.top) && (screen_y < map_area.bottom))
    {
...
TbPixel get_overhead_mapblock_color(MapSubtlCoord stl_x, MapSubtlCoord stl_y, PlayerNumber plyr_idx, TbPixel background)
{
    ...
    if ((((mapblk->flags & SlbAtFlg_Unexplored) != 0) || ((mapblk->flags & SlbAtFlg_TaggedValuable) != 0)) && ((game.play_gameturn & 4) != 0))
    {
    ...
    else if ((mapblk->flags & SlbAtFlg_IsRoom) != 0) // Room slab
    {
        ...
        else if (owner == game.neutral_player_num)
        {
    ...
    else
    {
        ...
        else if ((mapblk->flags & SlbAtFlg_Filled) != 0)
        {
            pixval = pixmap.ghost[background + 0x1000];
        }
        else if ((mapblk->flags & SlbAtFlg_IsDoor) != 0) // Door slab
        {
            ...
            else if ((game.play_gameturn & 1) && (thing->model == gui_door_type_highlighted))
            {
                pixval = player_highlight_colours[owner];
            }
            else if (door_is_hidden_to_player(thing, plyr_idx))
            {
                pixval = pixmap.ghost[background + 0x1000];
            }
            else if (thing->door.is_locked)
            {
                pixval = 79;
            }
            else
        ...
        else if ((mapblk->flags & SlbAtFlg_Blocking) == 0)
        {
            ...
            else if (slb->kind == SlbT_WATER)
            {
                pixval = 85;
            }
            else if (slb->kind == SlbT_PURPLE)
            {
                pixval = 255;
            }
            else if (owner == game.neutral_player_num)
    ...
    while (i != 0)
    {
        ...
        if (!thing_is_picked_up(thing))
        {
            ...
            if (thing_revealed(thing, plyr_idx))
            {
                ...
                else if ((game.play_gameturn & 4) == 0)
                {
...
void draw_map_level_name(void)
{
    ...
    else if (is_multiplayer_level(lvnum))
    {
        lv_name = level_name;
    }
    // Retrieving position
...
void draw_zoom_box(void)
{
    ...
    else if (player->minimap_zoom == 256)
    {
        draw_tiles = 9;
        subtile_unscaled = 12;
    }
    else if (player->minimap_zoom == 512)
    {
        draw_tiles = 12;
        subtile_unscaled = 9;
    }
    else if (player->minimap_zoom == 1024)
    {
        draw_tiles = 18;
        subtile_unscaled = 6;
    }
    else if (player->minimap_zoom == 2048)
    {
        draw_tiles = 36;
        subtile_unscaled = 3;
    }
    ...
    if ((stl_x < -draw_tiles_x / 2) || (stl_x >= gameadd.map_subtiles_x + 1 - draw_tiles_x / 2) || (stl_y < -draw_tiles_y / 2) || (stl_y >= gameadd.map_subtiles_y + 1 - draw_tiles_y / 2))
    {
        return;
    }
