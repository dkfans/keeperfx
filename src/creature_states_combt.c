    ...
    for (int p = PRIORITY_MAX; p >= 0; p--)
    {
        for (int i = 0; i < game.conf.crtr_conf.instances_count; i++)
        {
            ...
            if (inst_inf->priority < p) // Instances with low priority are used last.
                continue;
            if ((flag_is_set(inst_inf->flags, InstPF_SelfBuff)) && ((creature_instances_func_list[inst_inf->func_idx] != creature_cast_spell) || (!creature_affected_by_spell(thing, inst_inf->func_params[0]))))
            {
...
CrInstance get_self_spell_casting(const struct Thing *thing)
{
    ...
    for (int p = PRIORITY_MAX; p >= 0; p--)
    {
        for (int i = 0; i < game.conf.crtr_conf.instances_count; i++)
        {
            ...
            if (inst_inf->priority < p) // Instances with low priority are used last.
                continue;
            if ((creature_instances_func_list[inst_inf->func_idx] != creature_cast_spell) || (!creature_affected_by_spell(thing, inst_inf->func_params[0])))
            {
