    ...
    while (i != 0)
    {
        ...
        if (!thing_is_picked_up(thing))
        {
            if (thing->model == 24)//TODO CONFIG object model dependency, move to config
            {
                ...
                zmpos_x = thingadd->interp_minimap_pos_x / zoom;
                zmpos_y = thingadd->interp_minimap_pos_y / zoom;
...
int draw_overlay_traps(struct PlayerInfo *player, long units_per_px, long scaled_zoom, long basic_zoom)
{
    ...
    while (i != 0)
    {
        ...
        if (player->id_number == thing->owner)
        {
            ...
            zmpos_x = thingadd->interp_minimap_pos_x / scaled_zoom;
            zmpos_y = thingadd->interp_minimap_pos_y / scaled_zoom;
