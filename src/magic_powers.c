...
static TbResult magic_use_power_armageddon(PowerKind power_kind, PlayerNumber plyr_idx, struct Thing *thing, MapSubtlCoord stl_x, MapSubtlCoord stl_y, KeepPwrLevel power_level, unsigned long mod_flags)
{
    ...
    while (i != 0)
    {
        ...
        struct CreatureControl *cctrl;
        cctrl = creature_control_get_from_thing(thing);
        if (is_neutral_thing(thing) && !game.conf.rules[TODO_SO_ATM_0].magic.armageddon_teleport_neutrals) // Creatures unaffected by Armageddon.
        {
        ...
        else // Creatures teleported by Armageddon.
        {
            ...
            if (thing->owner == plyr_idx) {
                your_time_gap += game.conf.rules[TODO_SO_ATM_0].magic.armageddon_teleport_your_time_gap;
            } else {
                enemy_time_gap += game.conf.rules[TODO_SO_ATM_0].magic.armageddon_teleport_enemy_time_gap;
            }
        }
        // Per-thing code ends
        k++;
...
static void magic_power_hold_audience_update(PlayerNumber plyr_idx)
{
    ...
    dungeon = get_players_num_dungeon(plyr_idx);
    SYNCDBG(8,"Starting");
    if ( game.play_gameturn - dungeon->hold_audience_cast_turn <= game.conf.rules[TODO_SO_ATM_0].magic.hold_audience_time) {
        return;
    }
...
void process_magic_power_call_to_arms(PlayerNumber plyr_idx)
{
    ...
    if (!free)
    {
        if ((game.conf.rules[TODO_SO_ATM_0].game.allies_share_cta) && (players_are_mutual_allies(plyr_idx, slabmap_owner(slb))))
        {
            free = true;
        }
    }
