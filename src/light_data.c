...
DLLIMPORT char _DK_light_render_light_dynamic_1(struct Light *lgt, int radius, int a3, unsigned int max_1DD41_idx);
        ...
        if ( &game.lish.lighting_tables[game.lish.lighting_tables_count] > &game.lish.lighting_tables[0] )
        {
            do
            {
                ...
                if ( stl_x < map_subtiles_x && stl_y < map_subtiles_y )
                {
                    ...
                    {
                        light_render_light_sub1_sub1(lgt->mappos.x.val, lgt->mappos.y.val, v17, stl_x, stl_y, &shadow_limit_idx2, &shadow_limit_idx3);
