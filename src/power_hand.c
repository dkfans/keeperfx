...
TbBool power_hand_is_full(const struct PlayerInfo *player)
{
    const struct Dungeon *dungeon;
  dungeon = get_dungeon(player->id_number);
  return (dungeon->num_things_in_hand >= game.conf.rules[plyr_idx].game.max_things_in_hand);
}
...
void clear_things_in_hand(struct PlayerInfo *player)
{
  ...
  long i;
  dungeon = get_dungeon(player->id_number);
  for (i=0; i < game.conf.rules[plyr_idx].game.max_things_in_hand; i++)
...
void draw_mini_things_in_hand(long x, long y)
{
    ...
    {
        unsigned char ratio = (game.conf.rules[plyr_idx].game.max_things_in_hand / 2);
        if (game.conf.rules[plyr_idx].game.max_things_in_hand % 2)
        {
            ratio ++;
        }
        int icol = i % ratio;
