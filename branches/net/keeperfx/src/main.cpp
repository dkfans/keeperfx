...
void pannel_map_update(long x, long y, long w, long h)
{
    ...
    struct Thing *door_thing;
    int tile_value;
    int current_y;
    int map_y; // [sp+18h] [bp-20h]@2
    int the_slab;
    char *panel_map_tile;
    char *panel_map_ptr;
    int slab_index;
    ...
    panel_map_ptr = &panel_map_array[64 * y + x];
    player_id_mask = 1 << get_my_player()->id_number;
    current_y = y;
    ...
    do
    {
        if ( map_y >= 256 * 256 )
            break;
        current_x = x;
        for ( panel_map_tile = panel_map_ptr; current_x < x + w; ++panel_map_tile )
        {
            if ( current_x >= 256 )
                break;
            *(char *) &tile_value = 0;
            map_tile_flags = game.map[current_x + map_y].flags;
            if ( current_x / 3 < 85 )
            {
                if ( current_y / 3 < 85 )
                {
                    slab_index = 6 * (85 * map_to_slab[current_y] + map_to_slab[current_x]);
                    the_slab = game.slabmap[slab_index].slab;
                    weird_slab_result = game.slabmap[map_to_slab[((unsigned int)((unsigned long long)(current_x + map_y) >> 32) ^ (unsigned char)abs(current_x + map_y))
                        - ((unsigned long long)(current_x + map_y) >> 32)]
                        + 85 * map_to_slab[(current_x + map_y) / 256]].field_5 & 7;
                    if ( weird_slab_result > 6 ) //make weird slab result range from -3 to +3
                        weird_slab_result -= 3;
                    if ( map_tile_flags & 4 )
                    {
                        *(char *) tile_value = 3;
                    ...
                    {
                        if ( map_tile_flags & 0x80 )
                        {
                            *(char *) tile_value = 4;
                        }
                        else if ( map_tile_flags & 1 )
                        {
                            *(char *) tile_value = 5;
                        }
                        else if ( (unsigned char)player_id_mask & ((unsigned int)game.map[current_x + map_y].data >> 28) & 0xF )
                        {
                            if ( map_tile_flags & 2 )
                            {
                                tile_value = weird_slab_result + 6 * game.rooms[*(&game.slabmap[0].room_index + slab_index)].kind + 8;
                            }
                            else if ( game.slabmap[slab_index].slab )
                            {
                                if ( map_tile_flags & 0x20 )
                                {
                                    *(char *) tile_value = 1;
                                }
                                else if ( map_tile_flags & 0x40 )
                                {
                                    door_thing = get_door_for_position(current_x, current_y);
                                    if ( door_thing )
                                        tile_value = weird_slab_result + 6 * ((door_thing->byte_17 == 1) + 2 * door_thing->model) + 110;
                                    else
                                        ERRORLOG("No door for flagged position");
                                }
                                else if ( !(map_tile_flags & 0x10) )
                                {
                                    if ( the_slab == SlbT_LAVA )
                                        *(char *) tile_value = 6;
                                    else if ( the_slab == SlbT_WATER )
                                        *(char *) tile_value = 7;
                                    else
                                        tile_value = weird_slab_result + 170;
                                }
                            }
                            else
                            {
