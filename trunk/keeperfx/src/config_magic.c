...
{
  if ((pwkind < 0) || (pwkind >= magic_conf.power_types_count))
    return magic_conf.power_cfgstats[0].tooltip_str_idx;
  return magic_conf.power_cfgstats[pwkind].tooltip_str_idx;
}

long get_special_description_strindex(int spckind)
{
  if ((spckind < 0) || (spckind >= magic_conf.power_types_count))
    return magic_conf.special_cfgstats[0].tooltip_str_idx;
  return magic_conf.special_cfgstats[spckind].tooltip_str_idx;
}
