...
void frontnet_draw_text_bar(struct GuiButton *gbtn)
{
    ...
    spr = GetSprite(frontend_sprite, GFS_largearea_nx1_cor_l);
    LbSpriteDrawResized(pos_x, pos_y, fs_units_per_px, spr);
    pos_x += spr->SWidth * fs_units_per_px / 16;
    spr = GetSprite(frontend_sprite, GFS_largearea_nx1_tx5_c);
    for (i=0; i < 4; i++)
    {
        LbSpriteDrawResized(pos_x, pos_y, fs_units_per_px, spr);
        pos_x += spr->SWidth * fs_units_per_px / 16;
    }
    spr = GetSprite(frontend_sprite, GFS_largearea_nx1_cor_r);
    LbSpriteDrawResized(pos_x, pos_y, fs_units_per_px, spr);
}
...
void frontnet_draw_net_session_players(struct GuiButton *gbtn)
{
    ...
    const struct TbSprite *spr;
    spr = GetSprite(frontend_sprite, GFS_bullfrog_red_med);
    int fs_units_per_px;
    ...
    for (shift_y=0; shift_y < gbtn->height; shift_y += height, netplyr_idx++)
    {
        ...
        if (netplyr_idx >= net_number_of_enum_players)
            break;
        spr = GetSprite(frontend_sprite, GFS_bullfrog_red_med+netplyr_idx);
...
void frontnet_draw_alliance_box_tab(struct GuiButton *gbtn)
{
    ...
    LbSpriteDrawResized(pos_x, pos_y, fs_units_per_px, spr);
    pos_x += spr->SWidth*fs_units_per_px/16;
    spr = GetSprite(frontend_sprite, GFS_hugearea_thc_tx2_tc);
    LbSpriteDrawResized(pos_x, pos_y, fs_units_per_px, spr);
    pos_x += spr->SWidth*fs_units_per_px/16;
    ...
    {
        spr = GetSprite(frontend_sprite, GFS_bullfrog_blue_med);
        LbSpriteDrawResized(pos_x, pos_y, fs_units_per_px, spr);
        pos_x += spr->SWidth*fs_units_per_px/16;
    }
    if (net_number_of_enum_players > 2)
    {
        spr = GetSprite(frontend_sprite, GFS_bullfrog_green_med);
        LbSpriteDrawResized(pos_x, pos_y, fs_units_per_px, spr);
        pos_x += spr->SWidth*fs_units_per_px/16;
    }
    if (net_number_of_enum_players > 3)
    {
        spr = GetSprite(frontend_sprite, GFS_bullfrog_yellow_med);
        LbSpriteDrawResized(pos_x, pos_y, fs_units_per_px, spr);
    }
}
...
void frontnet_draw_alliance_grid(struct GuiButton *gbtn)
{
    ...
    pos_x = gbtn->scr_pos_x;
    spr = GetSprite(frontend_sprite, GFS_slidrect_indicator_std0);
    ...
    pos_x = gbtn->scr_pos_x;
    spr = GetSprite(frontend_sprite, GFS_slidrect_indicator_std1);
    for (netplyr_idx=0; netplyr_idx < NET_PLAYERS_COUNT; netplyr_idx++)
    {
        LbSpriteDrawResized(pos_x / pixel_size, pos_y / pixel_size, units_per_px, spr);
        pos_x += spr->SWidth * units_per_px / 16;
    ...
    pos_x = gbtn->scr_pos_x;
    spr = GetSprite(frontend_sprite, GFS_slidrect_indicator_std2);
    for (netplyr_idx=0; netplyr_idx < NET_PLAYERS_COUNT; netplyr_idx++)
    {
        LbSpriteDrawResized(pos_x / pixel_size, pos_y / pixel_size, units_per_px, spr);
        pos_x += spr->SWidth * units_per_px / 16;
...
void frontnet_draw_alliance_button(struct GuiButton *gbtn)
{
    ...
    if ((plyr1_idx == plyr2_idx) || (frontend_alliances & alliance_grid[plyr1_idx][plyr2_idx]))
      spr = GetSprite(frontend_sprite, GFS_scrollbar_indicator_std);
    else
      spr = GetSprite(frontend_sprite, GFS_slidrect_indicator_std1);
    int units_per_px;
...
void frontnet_draw_messages(struct GuiButton *gbtn)
{
    ...
    for (netmsg_id=net_message_scroll_offset; netmsg_id < net_number_of_messages; netmsg_id++)
    {
        ...
        spr = GetSprite(frontend_sprite, GFS_bullfrog_red_med+num_active);
...
void frontnet_draw_small_scroll_selection_box(struct GuiButton *gbtn, long font_idx, const char *text)
{
    ...
    LbSpriteDrawResized(pos_x, pos_y, fs_units_per_px, spr);
    pos_x += spr->SWidth * fs_units_per_px / 16;
    spr = GetSprite(frontend_sprite, GFS_largearea_xts_tx1_c);
    LbSpriteDrawResized(pos_x, pos_y, fs_units_per_px, spr);
    pos_x += spr->SWidth * fs_units_per_px / 16;
    LbSpriteDrawResized(pos_x, pos_y, fs_units_per_px, spr);
    pos_x += spr->SWidth * fs_units_per_px / 16;
    spr = GetSprite(frontend_sprite, GFS_largearea_xts_cor_r);
    LbSpriteDrawResized(pos_x, pos_y, fs_units_per_px, spr);
    if (text != NULL)
    {
        LbTextSetFont(frontend_font[font_idx]);
...
void frontnet_draw_text_cont_bar(struct GuiButton *gbtn)
{
    ...
    spr = GetSprite(frontend_sprite, GFS_largearea_nx2_cor_l);
    int fs_units_per_px;
    fs_units_per_px = spr->SHeight * units_per_px / 28;
    LbSpriteDrawResized(pos_x, pos_y, fs_units_per_px, spr);
    pos_x += spr->SWidth*fs_units_per_px/16;

    spr = GetSprite(frontend_sprite, GFS_largearea_nx2_tx5_c);
    for (netplyr_idx=0; netplyr_idx < NET_PLAYERS_COUNT; netplyr_idx++)
    {
        LbSpriteDrawResized(pos_x, pos_y, fs_units_per_px, spr);
        pos_x += spr->SWidth*fs_units_per_px/16;
    ...
    spr = GetSprite(frontend_sprite, GFS_largearea_nx2_cor_r);
    LbSpriteDrawResized(pos_x, pos_y, fs_units_per_px, spr);
}
