...
void MD5::update(uint1 *input, uint4 input_length)
{
    ...
    if (finalized) {  // so we can't update!
        throw new std::runtime_error("MD5::update:  Can't update a finalized digest!");
    }
...
{
    std::ifstream::char_type buffer[1024];
    int len;
...
void MD5::finalize()
{
    ...
    if (finalized) {
        throw new std::runtime_error("MD5::finalize:  Already finalized this digest!");
    }
...
unsigned char *MD5::raw_digest()
{
    ...
    if (!finalized) {
        delete[] s;
        throw new std::runtime_error("MD5::raw_digest:  Can't get digest if you haven't finalized it!");
    }
    memcpy(s, digest, 16);
    return s;
}
...
char *MD5::hex_digest()
{
    ...
    if (!finalized) {
        delete[] s;
        throw new std::runtime_error("MD5::hex_digest:  Can't get digest if you haven't finalized it!");
    }
...
/** @name Constants for MD5Transform routine.
