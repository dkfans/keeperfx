    ...
    else if ((game.flags_font & FFlg_AlexCheat) != 0)
    {
        ...
        else if (strcasecmp(parstr, "bug.toggle") == 0)
        {
            if (pr2str != NULL)
            {
                ...
                toggle_flag(game.conf.rules.classic_bugs_flags, flg);
                targeted_message_add(plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "%s %s", get_conf_parameter_text(rules_game_classicbugs_commands, bug), ((game.conf.rules.classic_bugs_flags & flg) != 0) ? "enabled" : "disabled");
                return true;
            }
        }
