    ...
    for (i = 0; i < LIGHTS_COUNT; i++)
    {
        ...
        if ((lgt->flags & LgtF_Allocated) != 0)
        {
            ...
            if ((lgt->shadow_index > 0) && (lgt->shadow_index < SHADOW_CACHE_COUNT))
            {
                ...
                else if (shadowcs[lgt->shadow_index] == 0)
                {
            ...
            else if ((lgt->flags & LgtF_Dynamic) != 0)
            {
    ...
    for (i = 1; i < THINGS_COUNT; i++)
    {
        ...
        if (thing_exists(thing))
        {
            if ((thing->light_id > 0) && (thing->light_id < LIGHTS_COUNT))
            {
                ...
                else if (lights[thing->light_id] == 0)
                {
...
void light_set_light_position(long lgt_id, struct Coord3d *pos)
{
    ...
    if (lgt->mappos.x.val != pos->x.val || pos->y.val != lgt->mappos.y.val || pos->z.val != lgt->mappos.z.val)
    {
        if ((lgt->flags & LgtF_Dynamic) == 0)
        {
            stat_light_needs_updating = 1;
            ...
            do
            {
                ...
                if (!subtile_coords_invalid(stl_x, stl_y))
                {
                    ...
                    if (game.lish.shadow_limits[shadow_limit_idx1])
                    {
                        ...
                        if ((!game.lish.shadow_limits[shadow_limit_idx2] || !game.lish.shadow_limits[shadow_limit_idx3]) && height > lgt->mappos.z.stl.num)
                        {
    ...
    if (game.lish.light_enabled)
    {
             ...
             lgt = &game.lish.lights[lgt->next_in_list])
        {
            if ((lgt->flags & (LgtF_Unkn80 | LgtF_Unkn08)) != 0)
            {
                ...
                if ((int)abs(half_width_x + startx - lgt->mappos.x.stl.num) < half_width_x + range && (int)abs(half_width_y + starty - lgt->mappos.y.stl.num) < half_width_y + range)
                {
...
void update_light_render_area(void)
{
    ...
    if (subtile_y > delta_y)
    {
        ...
        if (starty > gameadd.map_subtiles_y)
        {
            starty = gameadd.map_subtiles_y;
        }
    }
    else
    {
    ...
    if (subtile_x > delta_x)
    {
        ...
        if (startx > gameadd.map_subtiles_x)
        {
            startx = gameadd.map_subtiles_x;
        }
    }
    else
    {
    ...
    if (endy > gameadd.map_subtiles_y)
    {
        endy = gameadd.map_subtiles_y;
    }
    ...
    if (endx > gameadd.map_subtiles_x)
    {
        endx = gameadd.map_subtiles_x;
    }
    // Set the area
