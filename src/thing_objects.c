...
void destroy_food(struct Thing *foodtng)
{
    ...
    if (!room_is_invalid(room))
    {
        if (room_role_matches(room->kind, RoRoF_FoodSpawn) && (room->owner == foodtng->owner))
        {
            ...
            foodtng->food.life_remaining = game.conf.rules.food_life_out_of_hatchery;
        }
    }
...
static long food_moves(struct Thing *objtng)
{
    ...
    if (!dirct_ctrl)
    {
      ...
      else
      {
            ...
            {
                objtng->food.life_remaining = game.conf.rules.food_life_out_of_hatchery;
...
static TngUpdateRet object_update_dungeon_heart(struct Thing *heartng)
{
    ...
    if ((heartng->health > 0) && (game.conf.rules.dungeon_heart_heal_time != 0))
    {
        if ((game.play_gameturn % game.conf.rules.dungeon_heart_heal_time) == 0)
        {
            heartng->health += game.conf.rules.dungeon_heart_heal_health;
            if (heartng->health < 0)
            {
              heartng->health = 0;
            } else
...
{
    if (value >= game.conf.rules.gold_per_hoard)
        value = game.conf.rules.gold_per_hoard;
...
struct Thing *create_gold_hoarde(struct Room *room, const struct Coord3d *pos, GoldAmount value)
{
    ...
    GoldAmount wealth_size_holds = game.conf.rules.gold_per_hoard / get_wealth_size_types_count();
...
GoldAmount gold_object_typical_value(ThingModel tngmodel)
{
    ...
    {
      case ObjMdl_GoldChest:
          return game.conf.rules.chest_gold_hold;
      case ObjMdl_GoldPot:
          return game.conf.rules.pot_of_gold_holds;
      case ObjMdl_Goldl:
          return game.conf.rules.gold_pile_value;
      case ObjMdl_GoldBag:
          return game.conf.rules.bag_gold_hold;
      case ObjMdl_SpinningCoin:
          return game.conf.rules.gold_pile_maximum;
      default:
        break;
    }
    return 0;
