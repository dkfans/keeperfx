...
TbBool has_new_rooms_to_research(const struct Dungeon *dungeon)
{
    ...
    for (long resnum = 0; resnum < dungeon->research_num; resnum++)
    {
        ...
        if (rsrchval->rtyp == RsCat_Room)
        {
            if ((dungeonadd->room_buildable[rsrchval->rkind] & 1) == 0)
            {
                if ((dungeonadd->room_resrchable[rsrchval->rkind] == 1) || (dungeonadd->room_resrchable[rsrchval->rkind] == 2) || ((dungeonadd->room_resrchable[rsrchval->rkind] == 4) && (dungeonadd->room_buildable[rsrchval->rkind] & 2)))
                {
                    return true;
                }
            }
...
short researching(struct Thing *thing)
{
    ...
    struct CreatureControl *cctrl = creature_control_get_from_thing(thing);
    if ((game.play_gameturn - dungeon->last_research_complete_gameturn < 50) && ((game.play_gameturn + thing->index) & 0x03) == 0)
    {
