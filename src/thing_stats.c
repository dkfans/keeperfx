...
GoldAmount calculate_gold_digged_out_of_slab_with_single_hit(long damage_did_to_slab, const struct SlabMap *slb)
{
    ...
    GoldAmount gold = (damage_did_to_slab * game.conf.rules.gold_per_gold_block) / game.block_health[slbattr->block_health_index];
    // Returns gold-per-hit as an integer
    if (slb->kind == SlbT_GEMS)
    {
        gold = gold * game.conf.rules.gem_effectiveness / 100;
    }
    else if (slb->health == 0)
    // if the last hit deals the damage exactly, just drop a pile and the remainder
    {
        gold += (game.conf.rules.gold_per_gold_block % gold);
    }
    else if (slb->health < 0)
    ...
    {
        gold = game.conf.rules.gold_per_gold_block - (game.block_health[slbattr->block_health_index] / damage_did_to_slab) * gold;
