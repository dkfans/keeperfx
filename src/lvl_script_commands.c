...
static void reveal_map_location_process(struct ScriptContext *context)
{
    ...
    SYNCDBG(0, "Revealing location type %ld", target);
    long x = 0;
    long y = 0;
    ...
    if ((x == 0) && (y == 0))
    {
        WARNLOG("Can't decode location %ld", target);
        return;
    }
    if (r == -1)
    {
...
const struct CommandDesc command_desc[] = {
  ...
  {"ADD_CREATURE_TO_LEVEL_AND_INIT",    "PCANNN  ", Cmd_ADD_CREATURE_TO_LEVEL_AND_INIT, NULL, NULL},
