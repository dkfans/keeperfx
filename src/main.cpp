...
// Check for Wine
    ...
    if (result)
    {
        ...
        default:
            ...
            else if (((cpu_info.feature_intl >> 8) & 0x0F) < 0x06)
            {
                gpoly_enable_pentium_pro(false);
            }
            else
...
TbBool players_cursor_is_at_top_of_view(struct PlayerInfo *player)
{
    int i;
    i = player->work_state;
    if ((i == PSt_BuildRoom) || (i == PSt_PlaceDoor) || (i == PSt_PlaceTrap) || (i == PSt_SightOfEvil) || (i == PSt_Sell) || (i == PSt_PlaceTerrain) || (i == PSt_MkDigger) || (i == PSt_MkGoodCreatr) || (i == PSt_MkBadCreatr))
    {
        return true;
    }
...
TbBool engine_point_to_map(struct Camera *camera, long screen_x, long screen_y, long *map_x, long *map_y)
{
    ...
    *map_x = 0;
    *map_y = 0;
    if ((pointer_x >= 0) && (pointer_y >= 0) && (pointer_x < (player->engine_window_width / pixel_size)) && (pointer_y < (player->engine_window_height / pixel_size)))
    {
        ...
        else if (*map_y > subtile_coord(gameadd.map_subtiles_y, -1))
        {
        ...
        else if (*map_x > subtile_coord(gameadd.map_subtiles_x, -1))
        {
...
void update_creatr_model_activities_list(void)
{
    ...
    {
        if ((dungeon->owned_creatures_of_model[crmodel] > 0) && (crmodel != get_players_spectator_model(my_player_number)))
        {
            int i;
    ...
    for (crmodel = 1; crmodel < gameadd.crtr_conf.model_count; crmodel++)
    {
        if ((dungeon->owned_creatures_of_model[crmodel] <= 0) && (crmodel != get_players_special_digger_model(my_player_number)))
        {
            int i;
            for (i = 0; i < num_breeds; i++)
            {
    ...
    while (i != 0)
    {
        ...
        if ((!thing_is_picked_up(thing)) && (!thing_is_dragged_or_pulled(thing)))
        {
            ...
            if (crsound->index > 0)
            {
                ...
                {
                    if (((cctrl->distance_to_destination != 0) && ((int)thing->floor_height >= (int)thing->mappos.z.val)) || ((thing->movement_flags & TMvF_Flying) != 0))
                    {
...
short display_should_be_updated_this_turn(void)
{
    ...
    else if (((game.play_gameturn & 0x3F) == 0) ||
...
static TbBool wait_at_frontend(void)
{
    ...
    if ((start_params.operation_flags & GOF_SingleLevel) != 0)
    {
        ...
        if (!result)
        {
            ...
            else if (start_params.selected_campaign[0] != '\0')
            { // only show this log message if the user actually specified a campaign
...
short process_command_line(unsigned short argc, char *argv[])
{
    ...
    while (narg < argc)
    {
        ...
        else if (strcasecmp(parstr, "nocd") == 0) // kept for legacy reasons
        {
            WARNLOG("The -nocd commandline parameter is no longer functional. Game music from CD is a setting in keeperfx.cfg instead.");
        }
        else if (strcasecmp(parstr, "cd") == 0)
        {
            start_params.overrides[Clo_CDMusic] = true;
        }
        else if (strcasecmp(parstr, "1player") == 0)
        {
            start_params.one_player = 1;
            one_player_mode = 1;
        }
        else if ((strcasecmp(parstr, "s") == 0) || (strcasecmp(parstr, "nosound") == 0))
        {
            SoundDisabled = 1;
        }
        ...
        else if (strcasecmp(parstr, "packetsave") == 0)
        {
        ...
        else if (strcasecmp(parstr, "sessions") == 0)
        {
            narg++;
            LbNetwork_InitSessionsFromCmdLine(pr2str);
        }
        else if (strcasecmp(parstr, "alex") == 0)
        {
        ...
        else if (strcasecmp(parstr, "fixed_seed") == 0)
        {
            set_flag_byte(&start_params.autotest_flags, ATF_FixedSeed, true);
        }
        else if (strcasecmp(parstr, "tests") == 0)
        {
