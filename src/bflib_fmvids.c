...
{
    HMODULE hModule;
    hModule = LoadLibrary("SMACKW32");
    FARPROC proc;
    proc = GetProcAddress(hModule, "_SmackSoundUseMSS@4");
    if (proc == NULL)
    {
        ERRORLOG("Can't get address of SmackSoundUseMSS function; skipped.");
        return 0;
    }
...
struct SmackTag *SmackOpen(const char *name, unsigned int flags, int extrabuf)
{
    ...
    if (proc == NULL)
    {
        ERRORLOG("Can't get address of SmackOpen function; skipped.");
        return 0;
    }
...
void SmackSummary(struct SmackTag *smk, struct SmackSumTag *sum)
{
    ...
    if (proc == NULL)
    {
        ERRORLOG("Can't get address of SmackSummary function; skipped.");
        return;
    }
...
unsigned long SmackWait(struct SmackTag *smk)
{
    ...
    if (proc == NULL)
    {
        ERRORLOG("Can't get address of SmackWait function; skipped.");
        return 0;
    }
...
void SmackClose(struct SmackTag *smk)
{
    ...
    if (proc == NULL)
    {
        ERRORLOG("Can't get address of SmackClose function; skipped.");
        return;
    }
...
unsigned long SmackDoFrame(struct SmackTag *smk)
{
    ...
    if (proc == NULL)
    {
        ERRORLOG("Can't get address of SmackDoFrame function; skipped.");
        return 0;
    }
    return ((FARPROCP_U)proc)(smk);
}
...
void SmackNextFrame(struct SmackTag *smk)
{
    ...
    if (proc == NULL)
    {
        ERRORLOG("Can't get address of SmackNextFrame function; skipped.");
        return;
    }
    ((FARPROCP_V)(void *)proc)(smk);
}
                   ...
                   unsigned long Pitch, unsigned long destheight, const void *buf, unsigned long Flags)
{
    ...
    if (proc == NULL)
    {
        ERRORLOG("Can't get address of SmackToBuffer function; skipped.");
        return;
    }
...
void SmackGoto(struct SmackTag *smk, unsigned long frame)
{
    ...
    if (proc == NULL)
    {
        ERRORLOG("Can't get address of SmackGoto function; skipped.");
        return;
    }
...
void SmackSimulate(unsigned long sim)
{
    ...
    if (proc == NULL)
    {
        ERRORLOG("Can't get address of SmackSimulate function; skipped.");
        return;
    }
...
void copy_to_screen_px_ar_scale(unsigned char *src_buf, unsigned char *dst_buf, int src_width, int src_height, int flags)
{
    ...
    else // Otherwise, calculate the correct output size
    {
        ...
        {
            if (src_width == 320 && src_height == 200)
            { // make sure the source is 320x200
        ...
        {
            if ((flags & SMK_FullscreenStretch) != 0)
...
short play_smk_via_buffer(char *fname, int smkflags, int plyflags)
{
    ...
    while ((plyflags & 0x0400) || (smktag->Frames >= nframe))
    {
        ...
        do
        {
            ...
            if (((plyflags & SMK_NoStopOnUserInput) == 0) && (lbKeyOn[KC_ESCAPE] || lbKeyOn[KC_RETURN] || lbKeyOn[KC_SPACE] || lbDisplay.LeftButton))
            {
                SmackClose(smktag);
                LbMemoryFree(buf);
                return 2;
    ...
    while ((plyflags & 0x0400) || (smktag->Frames - 1 >= nframe))
    {
        ...
        do
        {
            ...
            if (((plyflags & SMK_NoStopOnUserInput) == 0) &&
                (lbKeyOn[KC_ESCAPE] || lbKeyOn[KC_RETURN] || lbKeyOn[KC_SPACE] || lbDisplay.LeftButton))
            {
                SmackClose(smktag);
                return 2;
            }
...
short play_smk_(char *fname, int smkflags, int plyflags)
{
    ...
    if ((smack_draw_callback != NULL) || ((plyflags & SMK_PixelDoubleWidth) != 0) || ((plyflags & SMK_InterlaceLine) != 0) || ((plyflags & SMK_PixelDoubleLine) != 0) || (LbScreenIsDoubleBufferred()))
    {
        ...
        *(unsigned short *)animation.field_C = hdim;
        ...
        for (h = hdim; h > 0; h--)
        {
            ...
            for (w = animation.header.width; w > 0;)
            {
                ...
                wendt = wend;
                if (animation.header.width == wend)
                {
                    continue;
                }
                if (w <= 0)
