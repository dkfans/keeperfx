...
void process_disease(struct Thing *creatng)
{
    ...
    if (CREATURE_RANDOM(creatng, 100) < game.conf.rules[plyr_idx].magic.disease_transfer_percentage)
    {
    ...
    if (((game.play_gameturn - cctrl->disease_start_turn) % game.conf.rules[plyr_idx].magic.disease_lose_health_time) == 0)
    {
        apply_damage_to_thing_and_display_health(creatng, game.conf.rules[plyr_idx].magic.disease_lose_percentage_health * cctrl->max_health / 100, cctrl->disease_caster_plyridx);
    }
}
