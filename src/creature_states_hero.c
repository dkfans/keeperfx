...
long creature_tunnel_to(struct Thing *creatng, struct Coord3d *pos, short speed)
{
    ...
    if ((pos->x.val != cctrl->navi.pos_final.x.val) || (pos->y.val != cctrl->navi.pos_final.y.val) || (pos->z.val != cctrl->navi.pos_final.z.val))
    {
    ...
    if (tnlret == 2)
    {
        ...
        else if (cctrl->instance_id == CrInst_NULL)
        {
            ...
            { // if this is false, that means the current tile must have changed to an undiggable wall
