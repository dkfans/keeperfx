        ...
        if (is_key_pressed(KC_GRAVE, KMod_DONTCARE))
        {
            if (thing->model == 20)
            {
                if (!(creature_affected_by_spell(thing, SplK_Freeze)))
                {
                    ...
                    {
                        // North
                        if ( (thing->move_angle_xy < 128) || (thing->move_angle_xy > 1920) )
                        {
                            stl_y--;
                        }
                        // Northeast
                        else if ( (thing->move_angle_xy >= 128) && (thing->move_angle_xy <= 384) )
                        {
                            stl_x++;
                            stl_y--;
                        }
                        // East
                        else if ( (thing->move_angle_xy > 384) && (thing->move_angle_xy < 640) )
                        {
                            stl_x++;
                        }
                        // Southeast
                        else if ( (thing->move_angle_xy >= 641) && (thing->move_angle_xy <= 896) )
                        {
                            stl_x++;
                            stl_y++;
                        }
                        // South
                        else if ( (thing->move_angle_xy >= 897) && (thing->move_angle_xy < 1152) )
                        {
                            stl_y++;
                        }
                        // Southwest
                        else if ( (thing->move_angle_xy >= 1152) && (thing->move_angle_xy < 1408) )
                        {
                            stl_x--;
                            stl_y++;
                        }
                        // West
                        else if ( (thing->move_angle_xy >= 1408) && (thing->move_angle_xy < 1664) )
                        {
                            stl_x--;
                        }
                        // Northwest
                        else if ( (thing->move_angle_xy >= 1664) && (thing->move_angle_xy <= 1920) )
                        {
                            stl_x--;
                            stl_y--;
                        }
                        ...
                        slappable = creature_is_slappable(whiptng, thing->owner);
                        i++;
                    }
                    while ((i <= 6) && (!slappable));
                    if ((!thing_is_invalid(thing)) && (slappable) )
                    {
                        if (whiptng != thing)
                        {
                            if (jonty_creature_can_see_thing_including_lava_check(thing, whiptng))
                            {
                                if (!creature_affected_by_slap(whiptng))
                                {
                                    thing_play_sample(thing, 87, NORMAL_PITCH, 0, 3, 0, 2, FULL_LOUDNESS);
                                    struct PlayerInfo* whip_plyr = get_player(thing->owner);
