...
void gui_area_instance_button(struct GuiButton *gbtn)
{
    ...
    if (!creature_instance_has_reset(ctrltng, curbtn_inst_id))
    {
        ...
        if ((ctrltng->alloc_flags & TAlF_IsControlled) != 0) {
            ...
            turns_progress = (long)game.play_gameturn - (long)cctrl->instance_use_turn[curbtn_inst_id] + inst_inf->fp_action_time - inst_inf->fp_time;
        } else {
            turns_required = inst_inf->reset_time;
            turns_progress = (long)game.play_gameturn - (long)cctrl->instance_use_turn[curbtn_inst_id] + inst_inf->action_time - inst_inf->time;
        }
