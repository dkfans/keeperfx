...
TbBool tag_cursor_blocks_sell_area(PlayerNumber plyr_idx, MapSubtlCoord stl_x, MapSubtlCoord stl_y, TbBool full_slab)
{
    ...
    {
        if ( ( ((subtile_is_sellable_room(plyr_idx, stl_x, stl_y)) || ((slabmap_owner(slb) == plyr_idx) && (slab_is_door(slb_x, slb_y) && subtile_is_sellable_door(stl_x, stl_y)))
            || ( (slabmap_owner(slb) == plyr_idx) && ( ((!full_slab) ? (subtile_has_sellable_trap_on(stl_x, stl_y)) : (slab_has_sellable_trap_on(slb_x, slb_y))) ) ) ) )
