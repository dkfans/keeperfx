...
long count_player_list_creatures_of_model(long thing_idx, ThingModel crmodel)
{
    ...
    while (i != 0)
    {
        ...
        i = cctrl->players_next_creature_idx;
        // Per creature code
        if ((crmodel == CREATURE_ANY) || (crmodel == CREATURE_NOT_A_DIGGER) || (thing->model == crmodel))
        {
            count++;
        }
        // Per creature code ends
...
struct Thing *get_player_list_nth_creature_of_model(long thing_idx, ThingModel crmodel, long crtr_idx)
{
    ...
    while (i != 0)
    {
      ...
      i = cctrl->players_next_creature_idx;
      // Per creature code
      if ((crmodel == 0) || (crmodel == CREATURE_ANY) || (crmodel == CREATURE_NOT_A_DIGGER) || (thing->model == crmodel))
          crtr_idx--;
      if (crtr_idx == -1)
          return thing;
      // Per creature code ends
