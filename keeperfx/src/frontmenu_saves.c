...
void gui_load_game(struct GuiButton *gbtn)
{
  ...
  if (!load_game(gbtn->in_group_idx))
  {
      ERRORLOG("Loading game %d failed; quitting.",(int)gbtn->in_group_idx);
...
void gui_save_game(struct GuiButton *gbtn)
{
  ...
  {
      slot_num = gbtn->in_group_idx%TOTAL_SAVE_SLOTS_COUNT;
...
void init_load_menu(struct GuiMenu *gmnu)
{
  ...
  SYNCDBG(6,"Starting");
  player = get_my_player();
  set_players_packet_action(player, PckA_UpdatePause, (short)true, (short)true);
