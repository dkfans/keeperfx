...
void update_battle_events(BattleIndex battle_id)
{
    ...
    while (i != 0)
    {
        ...
        i = cctrl->battle_prev_creatr;
        // Per thing code starts
        set_indexed_flag(owner_flags, thing->owner);
    ...
    for (i=0; i < PLAYERS_COUNT; i++)
    {
        if ((i == game.hero_player_num) || (i == game.neutral_player_num))
            continue;
        if (indexed_flag_is_set(owner_flags, i))
        {
            ...
            if (owner_flags == index_to_flag(i)) { // if the current player (i) is the only player in the fight
