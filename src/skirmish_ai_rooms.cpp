...
struct Pathfinder
{
    ...
    Pathfinder(PathNodeCompare open_cmp, PathNodeCompare closed_cmp) :
        ...
        goal_func(NULL), cost_func(NULL), heuristic_func(NULL), should_dig_func(NULL) { }
};
...
static struct PlayerRooms * rooms_of_player(int plyr_idx)
{
    assert(plyr_idx >= 0);
...
static struct SAI_Room * room_of_player(int plyr_idx, int room_id)
{
    struct PlayerRooms * rooms;
...
static void eval_players_room(int plyr, struct Room * room)
{
    ...
    if (any_rooms_marked_on_rect(rooms, r->rect)) {
        ...
        AIDBG(9, "Incomplete room found");
        set_room_state(rooms, r, SAI_ROOM_INCOMPLETE);
        mark_room(rooms, r);
    }
    else {
        AIDBG(9, "Active room found");
...
static int extend_layout(struct PlayerRooms * rooms)
{
    ...
    //mark rooms that should be extended this turn
    for (i = 0; i < SAI_MAX_ROOMS; ++i) {
        room = &rooms->array[i];
        if (room->state != SAI_ROOM_UNUSED && room->state != SAI_ROOM_BAD &&
                !(room->flags & SAI_ROOM_FLAG_EXTENDED)) {
            room->flags |= SAI_ROOM_FLAG_TEMP;
        }
        else {
            room->flags &= ~SAI_ROOM_FLAG_TEMP;
        }
    ...
    for (i = 0; i < SAI_MAX_ROOMS; ++i) {
        ...
        if (room->flags & SAI_ROOM_FLAG_TEMP) { //
...
static int look_for_room_of_size(struct PlayerRooms * rooms, enum RoomKinds kind,
    int desired_size, SAI_RoomState state)
{
    int w, h;
    int least;
    int most;
    ...
    side_length = (int) sqrt(desired_size);
    if (side_length * side_length < desired_size) {
        side_length += 1;
    }
...
int SAI_request_room(int plyr, enum RoomKinds kind, int size)
{
    ...
    res = look_for_room_of_size(rooms, kind, size, SAI_ROOM_EMPTY);
    id = max(id, res);

    while (id < 0) {
        res = look_for_room_of_size(rooms, kind, size, SAI_ROOM_UNDUG);
        id = max(id, res);
    ...
    if (id >= 0) {
        ...
        rooms->array[id].num_tiles_in_use = size;
    }
...
void SAI_set_room_kind(int plyr, int room_id, enum RoomKinds kind)
{
    ...
    room = room_of_player(plyr, room_id);
...
void SAI_set_tiles_in_use(int plyr, int room_id, int tiles_used)
{
    ...
    room = room_of_player(plyr, room_id);
    assert(room->state != SAI_ROOM_UNUSED);

    room->num_tiles_in_use = tiles_used;
}
