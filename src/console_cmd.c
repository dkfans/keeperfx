...
TbBool cmd_toggle_classic_bug(PlayerNumber plyr_idx, char * args)
{
    ...
    toggle_flag(game.conf.rules[TODO_SO_ATM_0].game.classic_bugs_flags, flg);
    targeted_message_add(MsgType_Player, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "%s %s", get_conf_parameter_text(rules_game_classicbugs_commands, bug), ((game.conf.rules[TODO_SO_ATM_0].game.classic_bugs_flags & flg) != 0) ? "enabled" : "disabled");
    return true;
}
...
TbBool cmd_exec(PlayerNumber plyr_idx, char * args)
{
    ...
    for (int i = 0; i < sizeof(console_commands) / sizeof(*console_commands); ++i) {
