...
void reveal_secret_door_to_player(struct Thing *doortng,PlayerNumber plyr_idx)
{
    ...
    {
        event_create_event(doortng->mappos.x.val, doortng->mappos.y.val, EvKind_SecretDoorDiscovered, plyr_idx, 0, 0);
        event_create_event(doortng->mappos.x.val, doortng->mappos.y.val, EvKind_SecretDoorSpotted, doortng->owner, 0, 0);
    }
