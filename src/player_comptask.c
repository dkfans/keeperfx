...
long task_check_room_dug(struct Computer2 *comp, struct ComputerTask *ctask)
{
    ...
    if (flag_is_set(gameadd.computer_chat_flags, CChat_TasksScarce)) {
        ...
        message_add_fmt(0, comp->dungeon->owner, "Now I can place the %s.",get_string(roomst->name_stridx));
    }
...
struct ComputerTask * able_to_build_room(struct Computer2 *comp, struct Coord3d *pos, RoomKind rkind, long width_slabs, long height_slabs, long max_slabs_dist, long perfect)
{
    ...
    if (!computer_task_invalid(ctask))
    {
        if (flag_is_set(gameadd.computer_chat_flags, CChat_TasksScarce)) {
            ...
            message_add_fmt(0, comp->dungeon->owner, "It is time to build %s.",get_string(roomst->name_stridx));
        }
...
TbBool create_task_move_creature_to_pos(struct Computer2 *comp, const struct Thing *thing, const struct Coord3d pos, CrtrStateId dst_state)
{
    ...
    if (flag_is_set(gameadd.computer_chat_flags, CChat_TasksFrequent)) {
        ...
        case CrSt_ImpImprovesDungeon:
            message_add_fmt(0, comp->dungeon->owner, "This %s should go claiming.",get_string(crconf->namestr_idx));
            break;
        case CrSt_ImpDigsDirt:
            message_add_fmt(0, comp->dungeon->owner, "This %s should go digging.",get_string(crconf->namestr_idx));
            break;
        case CrSt_ImpMinesGold:
            message_add_fmt(0, comp->dungeon->owner, "This %s should go mining.",get_string(crconf->namestr_idx));
            break;
        case CrSt_CreatureDoingNothing:
        case CrSt_ImpDoingNothing:
            message_add_fmt(0, comp->dungeon->owner, "This %s should stop doing that.",get_string(crconf->namestr_idx));
            break;
        case CrSt_CreatureSacrifice:
            if (thing->model == game.conf.rules.sacrifices.cheaper_diggers_sacrifice_model) {
                ...
                message_add_fmt(0, comp->dungeon->owner, "Sacrificing %s to reduce %s price.",get_string(crconf->namestr_idx),get_string(powerst->name_stridx));
                break;
            }
            message_add_fmt(0, comp->dungeon->owner, "This %s will be sacrificed.",get_string(crconf->namestr_idx));
            break;
        case CrSt_Torturing:
            message_add_fmt(0, comp->dungeon->owner, "This %s should be tortured.", get_string(crconf->namestr_idx));
            break;
        case CrSt_CreatureDoorCombat:
            message_add_fmt(0, comp->dungeon->owner, "This %s should attack a door.", get_string(crconf->namestr_idx));
            break;
        default:
            message_add_fmt(0, comp->dungeon->owner, "This %s should go there.",get_string(crconf->namestr_idx));
            break;
        }
    }
...
TbBool create_task_move_creatures_to_defend(struct Computer2 *comp, struct Coord3d *pos, long repeat_num, unsigned long evflags)
{
    ...
    if (flag_is_set(gameadd.computer_chat_flags, CChat_TasksScarce)) {
        message_add_fmt(0, comp->dungeon->owner, "Minions, defend this place!");
    }
...
TbBool create_task_move_creatures_to_room(struct Computer2 *comp, int room_idx, long repeat_num)
{
    ...
    if (flag_is_set(gameadd.computer_chat_flags, CChat_TasksScarce)) {
        ...
        if (room_exists(room)) {
            ...
            message_add_fmt(0, comp->dungeon->owner, "Time to put some creatures into %s.",get_string(roomst->name_stridx));
        } else {
            if (flag_is_set(gameadd.computer_chat_flags, CChat_TasksFrequent))
                message_add_fmt(0, comp->dungeon->owner, "Time to put some creatures into rooms.");
        }
    }
...
TbBool create_task_pickup_for_attack(struct Computer2 *comp, struct Coord3d *pos, long par3, long repeat_num)
{
    ...
    if (flag_is_set(gameadd.computer_chat_flags, CChat_TasksScarce)) {
        message_add_fmt(0, comp->dungeon->owner, "Minions, attack now!");
    }
...
TbBool create_task_magic_battle_call_to_arms(struct Computer2 *comp, struct Coord3d *pos, long cta_duration, long repeat_num)
{
    ...
    if (flag_is_set(gameadd.computer_chat_flags, CChat_TasksScarce)) {
        message_add_fmt(0, comp->dungeon->owner, "Minions, call to arms! Join the battle!");
    }
...
TbBool create_task_magic_support_call_to_arms(struct Computer2 *comp, struct Coord3d *pos, long cta_duration, long par3, long repeat_num)
{
    ...
    if (flag_is_set(gameadd.computer_chat_flags, CChat_TasksScarce)) {
        message_add_fmt(0, comp->dungeon->owner, "Minions, call to arms! Attack!");
    }
    ctask->ttype = CTT_MagicCallToArms;
    ctask->task_state = CTaskSt_None;
    ctask->created_turn = game.play_gameturn;
...
TbBool create_task_sell_traps_and_doors(struct Computer2 *comp, long num_to_sell, GoldAmount gold_up_to, TbBool allow_deployed)
{
    ...
    if (flag_is_set(gameadd.computer_chat_flags, CChat_TasksScarce)) {
        message_add_fmt(0, dungeon->owner, "I will sell some traps and doors.");
    }
...
TbBool create_task_move_gold_to_treasury(struct Computer2 *comp, long num_to_move, long gold_up_to)
{
    ...
    if (flag_is_set(gameadd.computer_chat_flags, CChat_TasksFrequent)) {
        message_add_fmt(0, comp->dungeon->owner, "Gold should not lay around outside treasury.");
    }
...
TbBool create_task_dig_to_attack(struct Computer2 *comp, const struct Coord3d startpos, const struct Coord3d endpos, PlayerNumber victim_plyr_idx, long parent_cproc_idx)
{
    ...
    if (flag_is_set(gameadd.computer_chat_flags, CChat_TasksScarce)) {
        message_add_fmt(0, comp->dungeon->owner, "Player %d looks like he need a kick.",(int)victim_plyr_idx);
    }
...
TbBool create_task_dig_to_neutral(struct Computer2 *comp, const struct Coord3d startpos, const struct Coord3d endpos)
{
    ...
    if (flag_is_set(gameadd.computer_chat_flags, CChat_TasksScarce)) {
        message_add_fmt(0, comp->dungeon->owner, "Localized neutral place, hopefully with loot.");
    }
...
TbBool create_task_dig_to_gold(struct Computer2 *comp, const struct Coord3d startpos, const struct Coord3d endpos, long parent_cproc_idx, long count_slabs_to_dig, long gold_lookup_idx)
{
    ...
    if (flag_is_set(gameadd.computer_chat_flags, CChat_TasksScarce)) {
        message_add_fmt(0, comp->dungeon->owner, "Time to dig more gold.");
    }
...
TbBool create_task_dig_to_entrance(struct Computer2 *comp, const struct Coord3d startpos, const struct Coord3d endpos, long parent_cproc_idx, long entroom_idx)
{
    ...
    if (flag_is_set(gameadd.computer_chat_flags, CChat_TasksScarce)) {
        ...
        message_add_fmt(0, comp->dungeon->owner, "I will take that %s.",get_string(roomst->name_stridx));
    }
...
TbBool create_task_slap_imps(struct Computer2 *comp, long creatrs_num)
{
    ...
    if (flag_is_set(gameadd.computer_chat_flags, CChat_TasksFrequent)) {
        message_add_fmt(0, comp->dungeon->owner, "Work harder, minions!");
    }
...
TbBool create_task_magic_speed_up(struct Computer2 *comp, const struct Thing *creatng, long splevel)
{
    ...
    if (flag_is_set(gameadd.computer_chat_flags, CChat_TasksScarce)) {
        message_add_fmt(0, comp->dungeon->owner, "I should speed up my fighters.");
    }
...
TbBool create_task_attack_magic(struct Computer2 *comp, const struct Thing *creatng, PowerKind pwkind, int repeat_num, int splevel, int gaction)
{
    ...
    if (flag_is_set(gameadd.computer_chat_flags, CChat_TasksScarce)) {
        ...
        message_add_fmt(0, comp->dungeon->owner, "Casting %s on %s!",get_string(powerst->name_stridx),get_string(crconf->namestr_idx));
    }
