...
{
    struct Coord3d pos;
    return find_first_valid_position_for_thing_anywhere_in_room(thing, room, &pos) && creature_can_navigate_to_with_storage(thing, &pos, flags);
}
...
long creature_turn_to_face_backwards(struct Thing *thing, struct Coord3d *pos)
{
    ...
    long angle = (get_angle_xy_to(&thing->mappos, pos) + LbFPMath_PI) & LbFPMath_AngleMask;
...
{
    struct CreatureControl *cctrl = creature_control_get_from_thing(thing);
    if (creature_instance_is_available(thing, CrInst_TELEPORT) && creature_instance_has_reset(thing, CrInst_TELEPORT) && (cctrl->instance_id == CrInst_NULL))
    {
...
TbBool thing_covers_same_blocks_in_two_positions(struct Thing *thing, struct Coord3d *pos1, struct Coord3d *pos2)
{
    ...
    if ((abs((pos2->x.val - nav_radius) - (pos1->x.val - nav_radius)) < COORD_PER_STL) && (abs((pos2->x.val + nav_radius) - (pos1->x.val + nav_radius)) < COORD_PER_STL) && (abs((pos2->y.val - nav_radius) - (pos1->y.val - nav_radius)) < COORD_PER_STL) && (abs((pos2->y.val + nav_radius) - (pos1->y.val + nav_radius)) < COORD_PER_STL) && (abs(pos2->z.val - pos1->z.val) < COORD_PER_STL) && (abs((thing->clipbox_size_z + pos2->z.val) - (thing->clipbox_size_z + pos1->z.val)) < COORD_PER_STL))
    {
        return true;
    }
    return false;
