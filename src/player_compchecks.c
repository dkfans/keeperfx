    ...
    if (able_to_use_power)
    {
        ...
        if (xy_walkable(stl_x, stl_y, dungeon->owner))
        {
            if ((gameadd.computer_chat_flags & CChat_TasksScarce) != 0) {
                ...
                message_add_fmt(0, comp->dungeon->owner, "My %s count is only %d, casting %s!",get_string(crconf->namestr_idx),(int)controlled_diggers,get_string(powerst->name_stridx));
            }
