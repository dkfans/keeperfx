...
TbBool process_prison_food(struct Thing *creatng, struct Room *room)
{
    ...
    if (thing_is_invalid(foodtng))
    {
        ...
        if ((offsetted_gameturn % 64 == 0) && thing_is_invalid(get_food_at_subtile_available_to_eat_and_owned_by(cctrl->moveto_pos.x.stl.num, cctrl->moveto_pos.y.stl.num, -1)))
        {
            foodtng = find_random_thing_in_room(TCls_Object, 10, room); // 10 = mature_food
            if (!thing_is_invalid(foodtng))
            {
                if (!is_thing_directly_controlled(foodtng) && line_of_room_move_2d(&creatng->mappos, &foodtng->mappos, room) && setup_person_move_to_position(creatng, foodtng->mappos.x.stl.num, foodtng->mappos.y.stl.num, 0))
                {
