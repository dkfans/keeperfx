...
{
    return (ulong)tng->mappos.x.val + (ulong)tng->mappos.y.val + (ulong)tng->mappos.z.val + (ulong)tng->move_angle_xy + (ulong)tng->owner;
}
...
void process_pause_packet(long curr_pause, long new_pause)
{
    ...
    for (long i = 0; i < PLAYERS_COUNT; i++)
    {
        ...
        if (player_exists(player) && (player->is_active == 1))
        {
            ...
            {
                if ((player->instance_num == PI_MapFadeTo) || (player->instance_num == PI_MapFadeFrom) || (player->instance_num == PI_CrCtrlFade) || (player->instance_num == PI_DirctCtrl) || (player->instance_num == PI_PsngrCtrl) || (player->instance_num == PI_DirctCtLeave) || (player->instance_num == PI_PsngrCtLeave))
                {
                    can = false;
                    break;
                }
    ...
    if (key == KC_BACK)
    {
        if (chpos > 0)
        {
            ...
            return true;
             ...
             ((chr >= 'A') && (chr <= 'Z')) ||
             ((chr >= '0') && (chr <= '9')) ||
             (chr == ' ') || (chr == '!') || (chr == ':') || (chr == ';') || (chr == '(') || (chr == ')') || (chr == '.') || (chr == '_') || (chr == '\'') || (chr == '+') || (chr == '=') || (chr == '-') || (chr == '"') || (chr == '?') || (chr == '/') || (chr == '#') || (chr == '<') || (chr == '>') || (chr == '^'))
    {
        if (chpos < maxlen)
        {
            message[chpos] = chr;
...
void process_players_creature_control_packet_action(long plyr_idx)
{
    ...
    case PckA_CtrlCrtrSetInstnc:
        ...
        else if (cctrl->instance_id == CrInst_NULL)
        {
...
// Debug code, to find packet errors
    ...
    // Clear all packets
    clear_packets();
    if (((game.system_flags & GSF_NetGameNoSync) != 0) || ((game.system_flags & GSF_NetSeedNoSync) != 0))
    {
        SYNCDBG(0, "Resyncing");
        resync_game();
    }
