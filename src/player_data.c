...
void set_player_state(struct PlayerInfo *player, short nwrk_state, long chosen_kind)
{
    ...
    if ((player->work_state != PSt_CreatrQuery) && (player->work_state != PSt_CreatrInfo) && (player->work_state != PSt_QueryAll) && (player->work_state != PSt_CreatrInfoAll) && (player->work_state != PSt_CtrlDirect) && (player->work_state != PSt_CtrlPassngr) && (player->work_state != PSt_FreeCtrlDirect) && (player->work_state != PSt_FreeCtrlPassngr))
    {
...
unsigned char rotate_mode_to_view_mode(unsigned char mode)
{
    ...
    {
    case 0:
        return PVM_IsoWibbleView;
    case 1:
        return PVM_IsoStraightView;
    case 2:
