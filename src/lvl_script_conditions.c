...
long get_condition_value(PlayerNumber plyr_idx, unsigned char valtype, unsigned char validx)
{
    ...
    case SVar_AVAILABLE_TRAP: // IF_AVAILABLE(TRAP)
        dungeonadd = get_dungeonadd(plyr_idx);
        return dungeonadd->mnfct_info.trap_amount_stored[validx % gameadd.trapdoor_conf.trap_types_count] + dungeonadd->mnfct_info.trap_amount_offmap[validx % gameadd.trapdoor_conf.trap_types_count];
    case SVar_AVAILABLE_DOOR: // IF_AVAILABLE(DOOR)
        dungeonadd = get_dungeonadd(plyr_idx);
        return dungeonadd->mnfct_info.door_amount_stored[validx % gameadd.trapdoor_conf.door_types_count] + dungeonadd->mnfct_info.door_amount_offmap[validx % gameadd.trapdoor_conf.door_types_count];
    ...
    case SVar_CONTROLS_CREATURE: // IF_CONTROLS(CREATURE)
        dungeon = get_dungeon(plyr_idx);
        return dungeon->owned_creatures_of_model[validx % gameadd.crtr_conf.model_count] - count_player_list_creatures_of_model_matching_bool_filter(plyr_idx, validx, creature_is_kept_in_custody_by_enemy_or_dying);
