...
short is_over_ensign(const struct LevelInformation *lvinfo, long scr_x, long scr_y)
{
    ...
    if ((map_x >= lvinfo->ensign_x - (spr_w >> 1)) && (map_x < lvinfo->ensign_x + (spr_w >> 1)) && (map_y > lvinfo->ensign_y - spr_h) && (map_y < lvinfo->ensign_y - (spr_h / 3)))
    {
        return true;
    }
    return false;
...
struct TbSprite *get_ensign_sprite_for_level(struct LevelInformation *lvinfo, int anim_frame)
{
    ...
    else if (lvinfo->options & LvOp_IsBonus)
    {
        switch (lvinfo->state)
        {
        case LvSt_Visible:
    ...
    else if (lvinfo->options & LvOp_IsExtra)
    {
        switch (lvinfo->state)
        {
        case LvSt_Visible:
    ...
    else if (lvinfo->options & LvOp_IsMulti) // Note that multiplayer flags have different file
    {
...
void frontzoom_to_point(long map_x, long map_y, long zoom)
{
    ...
    if (scr_x > lbDisplay.PhysicalScreenWidth - 1)
    {
        scr_x = lbDisplay.PhysicalScreenWidth - 1;
    }
    if (scr_x < 1)
    {
        scr_x = 1;
    ...
    if (scr_y > lbDisplay.PhysicalScreenHeight - 1)
    {
        scr_y = lbDisplay.PhysicalScreenHeight - 1;
    }
    if (scr_y < 1)
    {
        scr_y = 1;
...
TbBool frontmap_update_zoom(void)
{
    ...
    else if (map_info.fade_step == -FRONTMAP_ZOOM_STEP)
    {
...
TbBool frontmap_load(void)
{
    ...
    else if ((lvnum == first_singleplayer_level()) || (player->victory_state == VicS_LostLevel) || (player->victory_state == VicS_State3))
    {
...
TbBool test_hand_slap_collides(PlayerNumber plyr_idx)
{
    ...
    else if (nspck->param1 != SINGLEPLAYER_NOTSTARTED)
    {
...
void check_mouse_scroll(void)
{
    ...
    else if ((mx >= lbDisplay.PhysicalScreenWidth - 8) || ((is_game_key_pressed(Gkey_MoveRight, NULL, false)) || (is_key_pressed(KC_RIGHT, KMod_DONTCARE))))
    {
        map_info.velocity_x += 8;
        if (map_info.velocity_x < -48)
        {
    ...
    else if ((my >= lbDisplay.PhysicalScreenHeight - 8) || ((is_game_key_pressed(Gkey_MoveDown, NULL, false)) || (is_key_pressed(KC_DOWN, KMod_DONTCARE))))
    {
        map_info.velocity_y += 8;
        if (map_info.velocity_y < -48)
        {
...
TbBool frontnetmap_update_players(struct NetMapPlayersState *nmps)
{
    ...
    for (long i = 0; i < NET_PLAYERS_COUNT; i++)
    {
        ...
        else
        {
            ...
            else if (scratch[pckt_lvnum] > tmp2)
            {
