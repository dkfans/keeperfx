    ...
    MapSubtlCoord stl_x, MapSubtlCoord stl_y, int32_t *retdist)
{
    ...
    while (i != 0)
    {
      ...
      i = room->next_of_owner;
      // Per-room code
      int32_t dist = grid_distance(room->central_stl_x, room->central_stl_y, stl_x, stl_y);
...
struct Room * find_next_navigable_room_for_thing_with_capacity_and_closer_than(struct Thing *thing, int prev_room_idx, unsigned char nav_flags, int32_t used, int32_t *neardistance)
{
    uint32_t k = 0;
    int i = prev_room_idx;
    while (i != 0)
...
struct Room* find_nearest_navigable_room_of_kind_for_thing_with_capacity_and_closer_than(struct Thing* thing, PlayerNumber owner, RoomKind rkind, unsigned char nav_flags, int32_t used, int32_t * neardistance)
{
...
struct Room * find_nearest_navigable_room_for_thing_with_capacity_and_closer_than(struct Thing *thing, PlayerNumber owner, RoomRole rrole, unsigned char nav_flags, int32_t used, int32_t *neardistance)
{
 ...
 * @return Nearest room of given kind and owner, or invalid room if none found.
 */
struct Room *find_nearest_room_of_role_for_thing_with_used_capacity(struct Thing *thing, PlayerNumber owner, RoomRole rrole, unsigned char nav_flags, int32_t used)
{
    int32_t neardistance = INT32_MAX;
