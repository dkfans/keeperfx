        ...
        case 9: // KIND
            while ((k = recognize_conf_parameter(buf,&pos,len,cmpgn_map_cmnds_kind)) > 0)
            {
              ...
              case LvKind_IsSingle:
                if ((lvinfo->ensign_options & LvKind_IsSingle) == 0)
                {
                    if (add_single_level_to_campaign(&campaign,lvinfo->lvnum) >= 0)
                        n++;
                    lvinfo->ensign_options |= LvKind_IsSingle;
                } else
                    n++;
                break;
              case LvKind_IsMulti:
                if ((lvinfo->ensign_options & LvKind_IsMulti) == 0)
                {
                    if (add_multi_level_to_campaign(&campaign,lvinfo->lvnum) >= 0)
                        n++;
                    lvinfo->ensign_options |= LvKind_IsMulti;
                } else
                    n++;
                break;
              case LvKind_IsBonus:
                if ((lvinfo->ensign_options & LvKind_IsBonus) == 0)
                {
                    if (add_bonus_level_to_campaign(&campaign,lvinfo->lvnum) >= 0)
                        n++;
                    lvinfo->ensign_options |= LvKind_IsBonus;
                } else
                    n++;
                break;
              case LvKind_IsExtra:
                if ((lvinfo->ensign_options & LvKind_IsExtra) == 0)
                {
                    if (add_extra_level_to_campaign(&campaign,lvinfo->lvnum) >= 0)
                        n++;
                    lvinfo->ensign_options |= LvKind_IsExtra;
                } else
                    n++;
                break;
              case LvKind_IsFree:
                if ((lvinfo->ensign_options & LvKind_IsFree) == 0)
                {
                    if (add_freeplay_level_to_campaign(&campaign,lvinfo->lvnum) >= 0)
                        n++;
                    lvinfo->ensign_options |= LvKind_IsFree;
                } else
                    n++;
                break;
              }
