...
static LevelNumber frontnetmap_update_players(void)
{
    ...
    for (int32_t i = 0; i < MAX_NET_USERS; i++) {
        ...
        if ((action == NetAct_Slapping) && (nspck->action_par1 == NetLandSlap_HitFrame) && remote_player
          && (screen_packet_action(my_nspck) != NetAct_Limping)
          && hand_packet_overlaps_position(nspck, my_nspck->stored_data1, my_nspck->stored_data2)) {
        ...
        if (remote_player && (net_map_local.slap_miss_wait > 0) && (screen_packet_action(my_nspck) == NetAct_Slapping)
          && hand_packet_overlaps_position(my_nspck, nspck->stored_data1, nspck->stored_data2)) {
