...
static struct Thing *treasure_room_eats_gold_piles(struct Room *room, MapSlabCoord slb_x,  MapSlabCoord slb_y, struct Thing *hoardtng)
{
    ...
    if (gold_gathered > 0)
    {
        drop_gold_pile(gold_gathered, &pos, room->owner, false);
    }
    return hoardtng;
}
...
void count_gold_hoardes_in_room(struct Room *room)
{
    ...
    while (i > 0)
    {
        ...
        if (!thing_is_invalid(gldtng) && (gldtng->valuable.gold_stored > max_hoard_size_in_room))
        {
            ...
            drop_gold_pile(drop_amount, &pos, room->owner, false);
    ...
    case RoK_TREASURE:
        if (get_map_floor_filled_subtiles(mapblk) == 1)
        {
            ...
            if (!thing_is_invalid(gldtng))
            {
                ...
                drop_gold_pile(gldtng->valuable.gold_stored, &gldtng->mappos, room->owner, false);
                delete_thing_structure(gldtng, 0);
            }
        }
        break;
