...
int is_game_key_pressed(long key_id, long *val, TbBool ignore_mods)
{
    ...
    {
        if ((key_id == Gkey_RotateMod) && ((settings.kbkeys[Gkey_RotateMod].code == settings.kbkeys[Gkey_BestRoomSpace].code) ||
                                           (settings.kbkeys[Gkey_RotateMod].code == settings.kbkeys[Gkey_SquareRoomSpace].code)))
        {
            return 0;
        }
        if ((key_id == Gkey_SpeedMod) && ((settings.kbkeys[Gkey_SpeedMod].code == settings.kbkeys[Gkey_BestRoomSpace].code) ||
                                          (settings.kbkeys[Gkey_SpeedMod].code == settings.kbkeys[Gkey_SquareRoomSpace].code)))
        {
            return 0;
        }
        if ((key_id == Gkey_CrtrContrlMod) && ((settings.kbkeys[Gkey_CrtrContrlMod].code == settings.kbkeys[Gkey_BestRoomSpace].code) ||
...
long get_small_map_inputs(long x, long y, long zoom)
{
    ...
    if (((game.operation_flags & GOF_ShowGui) != 0) && (mouse_is_over_pannel_map(x, y) || grabbed_small_map))
    {
        ...
        if (do_left_map_click(x, y, curr_mx, curr_my, zoom) || do_right_map_click(x, y, curr_mx, curr_my, zoom) || do_left_map_drag(x, y, curr_mx, curr_my, zoom))
        {
            result = 1;
        }
    }
...
short get_global_inputs(void)
{
    ...
    if ((player->view_type == PVT_DungeonTop) && (((game.system_flags & GSF_NetworkActive) != 0) ||
...
long get_dungeon_control_action_inputs(void)
{
    ...
    else if ((player->work_state == PSt_PlaceTerrain) || (player->work_state == PSt_MkDigger) || (player->work_state == PSt_MkBadCreatr) || (player->work_state == PSt_MkGoodCreatr) || (player->work_state == PSt_KillPlayer) || (player->work_state == PSt_HeartHealth) || (player->work_state == PSt_StealRoom) ||
...
short get_creature_control_action_inputs(void)
{
    ...
    if (is_key_pressed(KC_NUMPADENTER, KMod_DONTCARE))
    {
        ...
        else if (toggle_instance_cheat_menu())
        {
            clear_key_pressed(KC_NUMPADENTER);
        }
    }
    ...
    if (numkey == -1)
    {
        ...
        else if (is_key_pressed(80, KMod_DONTCARE))
        {
            clear_key_pressed(80);
            numkey = 1;
        }
        else if (is_key_pressed(81, KMod_DONTCARE))
        {
            clear_key_pressed(81);
            numkey = 2;
        }
        else if (is_key_pressed(75, KMod_DONTCARE))
        {
            clear_key_pressed(75);
            numkey = 3;
        }
        else if (is_key_pressed(76, KMod_DONTCARE))
        {
            clear_key_pressed(76);
            numkey = 4;
        }
        else if (is_key_pressed(77, KMod_DONTCARE))
        {
            clear_key_pressed(77);
            numkey = 5;
        }
        else if (is_key_pressed(71, KMod_DONTCARE))
        {
            clear_key_pressed(71);
            numkey = 6;
        }
        else if (is_key_pressed(72, KMod_DONTCARE))
        {
            clear_key_pressed(72);
            numkey = 7;
        }
        else if (is_key_pressed(73, KMod_DONTCARE))
        {
            clear_key_pressed(73);
            numkey = 8;
        }
        else if (is_key_pressed(82, KMod_DONTCARE))
        {
    ...
    {
        set_players_packet_action(player, PckA_SwitchTeleportDest, 17, 0, 0, 0);
        ; // Call to Arms
...
void get_isometric_or_front_view_mouse_inputs(struct Packet *pckt, int rotate_pressed, int speed_pressed)
{
    ...
    if (game.frame_skip > 0)
    {
        ...
        if (global_frameskipTurn > frameskipMax)
        {
...
TbBool get_player_coords_and_context(struct Coord3d *pos, unsigned char *context)
{
    ...
    else if (!power_hand_is_empty(player))
    {
        *context = CSt_PowerHand;
        pos->x.val = (x << 8) + top_pointed_at_frac_x;
        pos->y.val = (y << 8) + top_pointed_at_frac_y;
    }
    else if (!subtile_revealed(x, y, player->id_number))
    {
        *context = CSt_PickAxe;
        pos->x.val = (x << 8) + top_pointed_at_frac_x;
        pos->y.val = (y << 8) + top_pointed_at_frac_y;
    }
    else if (((slb_x >= gameadd.map_tiles_x) || (slb_y >= gameadd.map_tiles_y)) && (!playeradd->one_click_lock_cursor))
    {
        *context = CSt_DefaultArrow;
        pos->x.val = (block_pointed_at_x << 8) + pointed_at_frac_x;
        pos->y.val = (block_pointed_at_y << 8) + pointed_at_frac_y;
    }
    else if (((slbattr->block_flags & (SlbAtFlg_Filled | SlbAtFlg_Digable | SlbAtFlg_Valuable)) != 0) || (playeradd->one_click_lock_cursor))
    {
        *context = CSt_PickAxe;
        pos->x.val = (x << 8) + top_pointed_at_frac_x;
        pos->y.val = (y << 8) + top_pointed_at_frac_y;
...
short get_gui_inputs(short gameplay_on)
{
    ...
    for (int gidx = 0; gidx < ACTIVE_BUTTONS_COUNT; gidx++)
    {
        ...
        if ((check_if_mouse_is_over_button(gbtn) && !game_is_busy_doing_gui_string_input()) || ((gbtn->gbtype == LbBtnT_Unknown6) && (gbtn->gbactn_1 != 0)))
        {
