...
static long cmd_comp_procs_click(struct GuiBox *gbox, struct GuiBoxOption *goptn, unsigned char btn, long *args)
{
    ...
    if (flag_is_set(cproc->flags, ComProc_Unkn0020))
        message_add_fmt(0, args[0], "resuming %s", cproc->name?cproc->name:"(null)");
    else
        message_add_fmt(0, args[0], "suspending %s", cproc->name?cproc->name:"(null)");
...
static long cmd_comp_checks_click(struct GuiBox *gbox, struct GuiBoxOption *goptn, unsigned char btn, long *args)
{
    ...
    if (flag_is_set(ccheck->flags, ComChk_Unkn0001))
        message_add_fmt(0, args[0], "resuming %s", ccheck->name?ccheck->name:"(null)");
    else
        message_add_fmt(0, args[0], "suspending %s", ccheck->name?ccheck->name:"(null)");
...
static TbBool cmd_magic_instance(PlayerNumber plyr_idx, char* creature_str, const char*  slot_str, char* instance_str)
{
    ...
    if (creature == -1)
    {
        targeted_message_add(0, 10, plyr_idx, GUI_MESSAGES_DELAY, "Invalid creature");
        return false;
    }
    int slot = atoi(slot_str);
    if (slot < 0 || slot > 9)
    {
        targeted_message_add(0, 10, plyr_idx, GUI_MESSAGES_DELAY, "Invalid slot");
        return false;
    }
    ...
    if (instance <= 0)
    {
        targeted_message_add(0, 10, plyr_idx, GUI_MESSAGES_DELAY, "Invalid instance");
        return false;
    }
...
TbBool cmd_exec(PlayerNumber plyr_idx, char *msg)
{
    ...
    {
      targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "Now time is %d, last loop time was %d",LbTimerClock(),last_loop_time);
      targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "clock is %d, requested fps is %d",clock(),game_num_fps);
      return true;
    }
    else if (strcasecmp(parstr, "fps") == 0)
    {
        if (pr2str == NULL)
        {
            ...
            targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "Framerate is %d fps", game_num_fps);
        }
        else
        {
    ...
    {
        targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "turn %ld", game.play_gameturn);
        return true;
    }
    ...
    else if (strcasecmp(parstr, "game.load") == 0)
    {
        ...
        if (is_save_game_loadable(slot_num))
        {
            ...
            {
                targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "Unable to load game %d", slot_num);
            }
        }
        else
        {
    ...
    {
        targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, PRODUCT_VERSION);
        return true;
    }
    else if (strcasecmp(parstr, "volume") == 0)
    {
        targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "%s: %d %s: %d", get_string(340), settings.sound_volume, get_string(341), settings.redbook_volume);
        return true;
    }
    ...
    else if ((game.flags_font & FFlg_AlexCheat) != 0)
    {
            ...
            if ((strcasecmp(pr2str, "scarce") == 0) || (strcasecmp(pr2str, "1") == 0))
            {
                for (int i = 0; i < PLAYERS_COUNT; i++)
                {
                    ...
                    if (player_exists(get_player(i)) && (!computer_player_invalid(comp)))
                        targeted_message_add(0, i, plyr_idx, GUI_MESSAGES_DELAY, "Ai model %d", (int) comp->model);
                }
                gameadd.computer_chat_flags = CChat_TasksScarce;
            }
            else if ((strcasecmp(pr2str, "frequent") == 0) || (strcasecmp(pr2str, "2") == 0))
            {
                targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "%s", pr2str);
                gameadd.computer_chat_flags = CChat_TasksScarce | CChat_TasksFrequent;
            }
            else
            {
                targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "none");
        ...
        else if (strcasecmp(parstr, "player.score") == 0)
        {
            ...
            {
                targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "Player %d score: %ld", id,
                                     dungeon->total_score);
                return true;
            }
        }
        else if (strcasecmp(parstr, "player.flag") == 0)
        {
            ...
            if (flg_id < SCRIPT_FLAGS_COUNT)
            {
                ...
                if (!dungeon_invalid(dungeon))
                {
                    ...
                    if (pr4str == NULL)
                    {
                        targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "Player %d flag %d value: %d", id,
        ...
        else if (strcasecmp(parstr, "comp.me") == 0)
        {
            ...
            {
                targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "unable to set assistant");
            }
            else
                targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "computer assistant is %d", atoi(pr2str));
            return true;
        }
        ...
        else if ((strcasecmp(parstr, "give.trap") == 0) || (strcasecmp(parstr, "trap.give") == 0))
        {
            ...
            targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "done!");
            return true;
        }
        ...
        else if (strcasecmp(parstr, "player.heart.health") == 0)
        {
            ...
            if (thing_is_dungeon_heart(thing))
            {
                ...
                if (pr3str == NULL)
                {
                    ...
                    targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY,
        ...
        else if (strcasecmp(parstr, "creature.job.set") == 0)
        {
            if (pr2str != NULL)
            {
                ...
                if (thing_is_creature(thing))
                {
                    ...
                    {
                        targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "Cannot do job %d.", new_job);
                        return true;
                    }
                }
            }
        ...
        else if (strcasecmp(parstr, "mapwho.info") == 0)
        {
            ...
            {
                targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "invalid location");
                return true;
            }
            player = get_player(plyr_idx);
            ...
            targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "first_thing:%d %s", thing_id,
                                 thing_class_and_model_name(thing->class_id, thing->model));
            targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "flags: %02x, data: %04lx", block->flags,
                                 block->data);
            targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "stl_x: %d, stl_y:%d", pos.x.stl.num,
                                 pos.y.stl.num);
        }
        else if (strcasecmp(parstr, "thing.info") == 0)
        {
            ...
            if (!thing_is_invalid(thing))
            {
                targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "next_on_map: %d, next_of_class: %d",
                                     thing->next_on_mapblk, thing->next_of_class);
                targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "health: %d", thing->health);
                targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "pos: %d %d %d", thing->mappos.x.stl.num,
                                     thing->mappos.y.stl.num,
                                     thing->mappos.z.stl.num);
                targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "%s",
                                     thing_class_and_model_name(thing->class_id, thing->model));
                return true;
            }
        }
        ...
        else if (strcasecmp(parstr, "cursor.pos") == 0)
        {
            ...
            targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "Cursor at %d, %d, %d",
        ...
        else if (strcasecmp(parstr, "thing.get") == 0)
        {
            ...
            if (!thing_is_invalid(thing))
            {
                targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "Got thing ID %d %s",
                                     thing->index,
                                     thing_class_and_model_name(thing->class_id, thing->model));
        ...
        else if (strcasecmp(parstr, "thing.show_id") == 0)
        {
        ...
        else if (strcasecmp(parstr, "thing.health") == 0)
        {
            ...
            if (!thing_is_invalid(thing))
            {
                ...
                {
                    targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "Thing ID: %d health: %d", thing->index, thing->health);
                }
                return true;
            }
        }
        else if (strcasecmp(parstr, "thing.move") == 0)
        {
            ...
            {
                targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "no thing selected");
            }
        }
        ...
        else if (strcasecmp(parstr, "room.get") == 0 )
        {
            ...
            if (room_exists(room))
            {
                targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "Got room ID %d", room->index);
        ...
        else if (strcasecmp(parstr, "room.health") == 0)
        {
            ...
            if (!room_is_invalid(room))
            {
                if (pr2str == NULL)
                {
                    targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "Room ID %d health: %d", room->index, room->health);
                    return true;
                }
                else
                {
        ...
        else if (strcasecmp(parstr, "slab.health") == 0)
        {
            ...
            if (!slabmap_block_invalid(slb))
            {
                if (pr2str == NULL)
                {
                    targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "Slab health: %d", slb->health);
                    return true;
                }
                else
                {
        ...
        else if (strcasecmp(parstr, "creature.pool.add") == 0)
        {
            if (pr3str != NULL)
            {
                ...
                if (crmodel == 0)
                {
                    targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "invalid creature model");
                    return true;
                }
        ...
        else if (strcasecmp(parstr, "zoomto") == 0)
        {
            if ( (pr2str != NULL) && (pr3str != NULL) )
            {
                ...
                {
                    targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "Co-ordinates specified are invalid");
                    return true;
                }
            }
        }
        else if (strcasecmp(parstr, "bug.toggle") == 0)
        {
            if (pr2str != NULL)
            {
                ...
                targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "%s %s", get_conf_parameter_text(rules_game_classicbugs_commands, bug), ((game.conf.rules.game.classic_bugs_flags & flg) != 0) ? "enabled" : "disabled");
                return true;
            }
        }
        else if (strcasecmp(parstr, "actionpoint.pos") == 0)
        {
            if (pr2str != NULL)
            {
                ...
                if (action_point_exists_idx(ap))
                {
                    ...
                    targeted_message_add(0, plyr_idx, plyr_idx, GUI_MESSAGES_DELAY, "Action Point %d X: %d Y: %d", ap, actionpt->mappos.x.stl.num, actionpt->mappos.y.stl.num);
                    return true;
                }
            }
        }
