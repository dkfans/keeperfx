...
DLLIMPORT long _DK_creature_can_have_combat_with_creature(const struct Thing *fightng, const struct Thing *enmtng, long dist, long a4, long a5);
...
long get_combat_distance(const struct Thing *thing, const struct Thing *enmtng)
{
    ...
    avgc = (enmtng->sizexy + thing->sizexy) / 2;
    if (dist < avgc)
        return 0;
    return dist - avgc;
}
...
long creature_can_have_combat_with_creature(struct Thing *fightng, struct Thing *enmtng, long dist, long a4, long set_combat)
{
...
TbBool remove_ranged_attacker(struct Thing *fighter, struct Thing *enmtng)
{
    ...
    {
        if (!remove_ranged_combat_attacker(enmtng, fighter->index)) {
...
TbBool remove_melee_attacker(struct Thing *fighter, struct Thing *enmtng)
{
    struct CreatureControl *figctrl;
    ...
    {
        if (!remove_melee_combat_attacker(enmtng, fighter->index)) {
            ERRORLOG("Cannot remove attacker - not in %s index %d opponents",thing_model_name(enmtng),(int)enmtng->index);
        }
    } else {
        WARNLOG("Cannot remove attacker - the %s index %d has no opponents",thing_model_name(enmtng),(int)enmtng->index);
...
TbBool set_creature_combat_state(struct Thing *fighter, struct Thing *enemy, long combat_kind)
{
    ...
    if ( (crstat->attack_preference == PrefAttck_Ranged) && creature_has_ranged_weapon(fighter) )
    {
        if ( add_ranged_attacker(fighter, enemy) )
        {
            play_creature_sound(fighter, 11, 3, 0);
...
long check_for_valid_combat(struct Thing *thing, struct Thing *enmtng)
{
    ...
    if (((game.play_gameturn + thing->index) & 7) == 0) {
        ...
        valid_combat = creature_can_have_combat_with_creature(thing, enmtng, dist, 1, 1);
    }
    return valid_combat;
}
...
void creature_in_ranged_combat(struct Thing *thing)
{
    ...
    {
        set_start_state(thing);
        SYNCDBG(9,"The %s index %d cannot choose ranged offensive weapon",thing_model_name(thing),(int)thing->index);
        return;
    }
