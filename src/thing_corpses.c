...
void remove_body_from_graveyard(struct Thing *thing)
{
    ...
    {
        if (dungeon->bodies_rotten_for_vampire >= game.conf.rules[plyr_idx].rooms.bodies_for_vampire) {
            dungeon->bodies_rotten_for_vampire -= game.conf.rules[plyr_idx].rooms.bodies_for_vampire;
...
TngUpdateRet update_dead_creature(struct Thing *thing)
{
    ...
    if ((thing->alloc_flags & TAlF_IsDragged) == 0)
    {
        ...
        if (corpse_laid_to_rest(thing))
        {
            ...
            {
                if (game.play_gameturn - thing->creation_turn > game.conf.rules[plyr_idx].creature.body_remains_for) {
                    delete_thing_structure(thing, 0);
                    return TUFRet_Deleted;
                }
            }
        } else
        {
            ...
            if (((corpse_age > game.conf.rules[plyr_idx].creature.body_remains_for) ||(!corpse_is_rottable(thing) && (corpse_age > VANISH_EFFECT_DELAY)))
