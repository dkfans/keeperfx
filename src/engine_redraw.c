...
void make_camera_deviations(struct PlayerInfo *player, struct Dungeon *dungeon)
{
    ...
    if ((dungeon->camera_deviate_quake != 0) || (dungeon->camera_deviate_jump != 0))
    {
        ...
        else if (x > (gameadd.map_subtiles_x + 1) * COORD_PER_STL - 1)
        {
        ...
        else if (y > (gameadd.map_subtiles_y + 1) * COORD_PER_STL - 1)
        {
...
void process_dungeon_top_pointer_graphic(struct PlayerInfo *player)
{
    ...
    case PSt_CtrlDungeon:
        ...
        case CSt_PowerHand:
            ...
            else if (((player->input_crtr_query) && !thing_is_invalid(thing)) && (dungeon->things_in_hand[0] != player->thing_under_hand) && can_thing_be_queried(thing, player->id_number))
            {
