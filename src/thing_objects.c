...
static long food_moves(struct Thing *objtng)
{
    ...
    if (objtng->food.byte_15 <= 0)
    {
        if (objtng->food.byte_15 == 0)
        {
            ...
            set_thing_draw(objtng, 820, -1, -1, -1, 0, ODC_DrawClass2);
            if (dirct_ctrl) {
        ...
        {
            set_thing_draw(objtng, 819, -1, -1, -1, 0, ODC_DrawClass2);
...
void set_call_to_arms_as_birthing(struct Thing *objtng)
{
    ...
    set_thing_draw(objtng, ctagfx->birth_anim_idx, 256, objdat->sprite_size_max, 0, frame, ODC_DrawClass2);
...
void set_call_to_arms_as_dying(struct Thing *objtng)
{
    ...
    struct CallToArmsGraphics* ctagfx = &call_to_arms_graphics[objtng->owner];
    struct Objects* objdat = get_objects_data_for_thing(objtng);
    set_thing_draw(objtng, ctagfx->leave_anim_idx, 256, objdat->sprite_size_max, 0, frame, ODC_DrawClass2);
...
static TngUpdateRet object_update_call_to_arms(struct Thing *thing)
{
    ...
    case CTAOL_Birthing:
        if (thing->max_frames - 1 <= thing->current_frame)
        {
            ...
            set_thing_draw(thing, ctagfx->alive_anim_idx, 256, objdat->sprite_size_max, 0, 0, ODC_DrawClass2);
            return 1;
        }
        break;
    case CTAOL_Alive:
    ...
    case CTAOL_Rebirthing:
    {
        if (thing->max_frames - 1 == thing->current_frame)
        {
            ...
            set_thing_draw(thing, ctagfx->birth_anim_idx, 256, objdat->sprite_size_max, 0, 0, ODC_DrawClass2);
