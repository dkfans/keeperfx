...
TbBool skip_conf_spaces(const char *buf, long *pos, long buflen)
{
    while ((*pos) < buflen)
    {
        if ((buf[*pos] != ' ') && (buf[*pos] != '\t') && (buf[*pos] != 26) && ((unsigned char)buf[*pos] >= 7))
        {
            break;
        }
        (*pos)++;
    ...
    while (commands[i].num > 0)
    {
        ...
        if (strnicmp(buf + (*pos), commands[i].name, cmdname_len) == 0)
        {
            ...
            if ((*pos) < buflen)
            {
                ...
                if ((buf[(*pos)] != ' ') && (buf[(*pos)] != '\t') && (buf[(*pos)] != '=') && ((unsigned char)buf[(*pos)] >= 7))
                {
                    (*pos) -= cmdname_len;
                    i++;
                    continue;
                ...
                while ((buf[*pos] == ' ') || (buf[*pos] == '\t') || (buf[*pos] == '=') || ((unsigned char)buf[*pos] < 7))
                {
                    (*pos)++;
                    if ((*pos) >= buflen)
                    {
...
int recognize_conf_parameter(const char *buf, long *pos, long buflen, const struct NamedCommand commands[])
{
    ...
    while (commands[i].name != NULL)
    {
        ...
        if (strncasecmp(&buf[(*pos)], commands[i].name, par_len) == 0)
        {
            ...
            if ((buf[(*pos) + par_len] == ' ') || (buf[(*pos) + par_len] == '\t') || ((unsigned char)buf[(*pos) + par_len] < 7))
            {
                (*pos) += par_len + 1;
                return commands[i].num;
            }
...
short calculate_moon_phase(short do_calculate, short add_to_log)
{
    ...
    else if ((phase_of_moon > -0.1) && (phase_of_moon < 0.1))
    {
        if (add_to_log)
        {
    ...
    else if ((phase_of_moon < -0.95) || (phase_of_moon > 0.95))
    {
        if (add_to_log)
        {
    ...
    else if ((phase_of_moon < -0.9) || (phase_of_moon > 0.9))
    {
        if (add_to_log)
        {
...
{
    if (sp_lvnum == SINGLEPLAYER_FINISHED)
    {
        return SINGLEPLAYER_FINISHED;
    }
    if (sp_lvnum == SINGLEPLAYER_NOTSTARTED)
    {
...
LevelNumber prev_singleplayer_level(LevelNumber sp_lvnum)
{
    ...
    if (sp_lvnum == SINGLEPLAYER_FINISHED)
    {
        return last_singleplayer_level();
    }
    if (sp_lvnum < 1)
    {
        return LEVELNUMBER_ERROR;
...
{
    if (mp_lvnum == SINGLEPLAYER_FINISHED)
    {
        return SINGLEPLAYER_FINISHED;
    }
    if (mp_lvnum == SINGLEPLAYER_NOTSTARTED)
    {
        return first_multiplayer_level();
    }
    if (mp_lvnum < 1)
    {
        return LEVELNUMBER_ERROR;
...
{
    if (ex_lvnum == SINGLEPLAYER_FINISHED)
    {
        return SINGLEPLAYER_FINISHED;
    }
    if (ex_lvnum == SINGLEPLAYER_NOTSTARTED)
    {
        return first_extra_level();
    }
    if (ex_lvnum < 1)
    {
        return LEVELNUMBER_ERROR;
...
{
    if (fp_lvnum == SINGLEPLAYER_FINISHED)
    {
        return SINGLEPLAYER_FINISHED;
    }
    if (fp_lvnum == SINGLEPLAYER_NOTSTARTED)
    {
        return first_freeplay_level();
    }
    if (fp_lvnum < 1)
    {
        return LEVELNUMBER_ERROR;
...
{
    if (is_singleplayer_level(lvnum) || is_bonus_level(lvnum) || is_extra_level(lvnum) || is_multiplayer_level(lvnum))
    {
        return true;
    }
    return false;
