...
void anger_set_creature_anger_f(struct Thing *creatng, long annoy_lv, AnnoyMotive reason, const char *func_name)
{
    ...
    else if (annoy_lv > (3 * crstat->annoy_level))
    {
        annoy_lv = (3 * crstat->annoy_level);
    }
    else if (annoy_lv > 65534)
    {
        annoy_lv = 65534;
    }
...
{
    if (thing_is_creature(victng) && (victng->index != creatng->index) && !creature_has_job(victng, job_kind) && !creature_is_kept_in_custody(victng) && !creature_is_being_unconscious(victng) && !creature_is_dying(victng) && !creature_is_doing_anger_job(victng))
    {
