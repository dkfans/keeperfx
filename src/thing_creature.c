...
struct Thing *kill_creature(struct Thing *creatng, struct Thing *killertng, PlayerNumber killer_plyr_idx, CrDeathFlags flags)
{
    ...
    if (!dungeon_invalid(dungeon))
    {
        dungeon->hates_player[killertng->owner] += game.conf.rules[dungeon->owner].creature.fight_hate_kill_value;
    }
...
TbBool creature_count_below_map_limit(TbBool temp_creature)
{
    ...
    return ((game.thing_lists[TngList_Creatures].count - temp_creature) < game.conf.rules[TODO_SO_ATM_0].game.creatures_count);  //todo Per player value or not?
}
