    ...
    for (i = 0; i < PLAYERS_COUNT; i++)
    {
        ...
        for (long n = 0; n < COMPUTER_SPARK_POSITIONS_COUNT; n++)
        {
            ...
            if ((pos->x.val > 0) && (pos->y.val > 0))
            {
                ...
                else if (find_from_task_list_by_subtile(dungeon->owner, pos->x.stl.num, pos->y.stl.num) >= 0)
                {
                    pos->x.val = 0;
                }
                else
...
TbBool setup_a_computer_player(PlayerNumber plyr_idx, long comp_model)
{
    ...
    long i;
    if ((plyr_idx >= PLAYERS_COUNT) || (plyr_idx == game.hero_player_num) || (plyr_idx == game.neutral_player_num))
    {
...
void process_computer_players2(void)
{
    ...
    else if (gameadd.turn_last_checked_for_gold > game.play_gameturn)
    {
