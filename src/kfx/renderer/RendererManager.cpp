...
{
    if (s_active_renderer == nullptr || desc == nullptr || desc->src == nullptr)
        return 0;
...
{
    if (s_active_renderer != nullptr && spr != nullptr)
        s_active_renderer->PresentHugeSprite(spr, sp_len, x_shift, y_shift, units_per_px);
}
                                ...
                                int dst_x, int dst_y, int tile_w, int tile_h)
{
    if (s_active_renderer != nullptr && tile_block_ids != nullptr)
        s_active_renderer->SubmitZoomBoxTiles(tile_block_ids, tiles_x, tiles_y,
...
void WorldViewRenderer_BeginWorldPass(int w, int h, int vp_x, int vp_y)
{
    IWorldViewRenderer* world = active_world_renderer();
    if (world != nullptr)
        world->BeginWorldPass(w, h, vp_x, vp_y);
}
    ...
    short angle, unsigned char sprgroup, int32_t scale, TbDrawFlagsMask draw_flags)
{
    ICursorLayer* cursor = active_cursor_layer();
    if (cursor != nullptr)
        cursor->SubmitKeeperHandSprite(x, y, kspr_base, angle, sprgroup, scale, draw_flags);
}
