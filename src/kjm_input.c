...
void update_mouse(void)
{
    ...
    lbInkey = lbDisplay.MiddleButton ? KC_MOUSE3 : wheel_scrolled_down ? KC_MOUSEWHEEL_DOWN
                                               : wheel_scrolled_up     ? KC_MOUSEWHEEL_UP
                                                                       : lbInkey;
}
...
long set_game_key(long key_id, unsigned char key, unsigned int mods)
{
    ...
    {
        if (((mods & KMod_SHIFT) && (mods & KMod_CONTROL)) || ((mods & KMod_SHIFT) && (mods & KMod_ALT)) || ((mods & KMod_CONTROL) && (mods & KMod_ALT)))
        {
            return 0;
        }
