...
static TbBool find_place_to_put_door_around_room(const struct Room *room, struct Coord3d *pos, PlayerNumber plyr_idx, ThingModel doorkind)
{
    ...
    for (long n = 0; n < SMALL_AROUND_SLAB_LENGTH; n++)
    {
        ...
        for (i = 4; i > 0; i--)
        {
            ...
            if (tag_cursor_blocks_place_door(room->owner, slab_subtile_center(slb_x), slab_subtile_center(slb_y),doorkind)) {
                break;
            }
...
long computer_check_for_place_door(struct Computer2 *comp, struct ComputerCheck * check)
{
    ...
    for (ThingModel doorkind = game.conf.trapdoor_conf.door_types_count; doorkind > 1; doorkind--)
    {
        ...
        while (i != 0)
        {
            ...
            pos.y.val = 0;
            pos.z.val = 0;
            if (find_place_to_put_door_around_room(room, &pos, dungeon->owner,doorkind))
            {
